...
// Per-layer fp16 sink registry for sink dispatch.
...
static void tq_init_rotations(void) {
    ...
    // Initialize per-layer/per-head registries to the default split.
...
void tq_lock_outliers_from_accum(int n_layers) {
    ...
    for (int l = 0; l < n_layers && l < TQ_MAX_LAYERS; l++) {
        for (int h = 0; h < TQ_MAX_HEADS; h++) {
            tq_detect_outliers_for_head(l, h, 1);
            tq_detect_outliers_for_head(l, h, 0);
        }
...
// ---------------------------------------------------------------------------
// Public accessors for rotation matrices (used by Metal/CUDA)
// ---------------------------------------------------------------------------
...
void tq_get_channel_perm(int layer, int head, int is_k, uint8_t * perm128) {
    ...
    for (int i = 0; i < 32; i++) {
        perm128[i] = (uint8_t) outlier[i];
    }
    for (int i = 0; i < 96; i++) {
        perm128[32 + i] = (uint8_t) regular[i];
    }
}
...
// ---------------------------------------------------------------------------
// TQV had_mse4: V cache 4-bit MSE, per-block norm
// ---------------------------------------------------------------------------
...
void dequantize_row_tqv_had_mse4(const block_tqv_had_mse4 * GGML_RESTRICT x, float * GGML_RESTRICT y, int64_t k) {
    ...
    for (int64_t i = 0; i < nb; i++) {
        ...
        tq_fwht(rot, TQ_DIM);
        for (int j = 0; j < TQ_DIM; j++) {
            y[i * TQK_BLOCK_SIZE + j] = norm * rot[j];
        }
    }
}

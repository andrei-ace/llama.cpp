...
// Used by 6hi_3lo outlier subset (32 channels, 32 centroids)
...
// QJL uses FWHT (Hadamard) as projection for all power-of-2 dims (32, 128).
// O(n log n), no matrix storage needed, enables trivial per-element Metal FA correction.
// Non-power-of-2 dims (96) use on-the-fly Gaussian PRNG.
...
// 5-bit pack/unpack: 8 values in 5 bytes (j*5 bits from start)
static inline void pk5(uint8_t * q, int j, int v) {
    int bit = j * 5;
    int byte0 = bit / 8;
    int shift = bit % 8;
    q[byte0] |= (uint8_t)((v & 0x1F) << shift);
    if (shift > 3) { // spans two bytes
        q[byte0 + 1] |= (uint8_t)((v & 0x1F) >> (8 - shift));
    }
}
...
// ---------------------------------------------------------------------------
// TQK 6hi_3lo_had: 32/96 split, 5-bit MSE + 1-bit QJL on outliers, 3-bit MSE on regulars
// Same as 5hi_3lo_had but with 32 centroids (5-bit) on hi instead of 16 (4-bit)
// ---------------------------------------------------------------------------
...
void quantize_row_tqk_6hi_3lo_had_ref(const float * GGML_RESTRICT x, block_tqk_6hi_3lo * GGML_RESTRICT y, int64_t k) {
    ...
    for (int64_t i = 0; i < nb; i++) {
        ...
        tq_fwht(hi_rot, TQ_DIM_HI);
        memcpy(lo_rot, lo_raw, sizeof(lo_rot));
        tq_fwht(lo_rot,       TQ_DIM_HI);
        tq_fwht(lo_rot + 32,  TQ_DIM_HI);
        tq_fwht(lo_rot + 64,  TQ_DIM_HI);
        ...
        // 5-bit MSE for hi (32 centroids, d=32)
        memset(y[i].qs_hi, 0, sizeof(y[i].qs_hi));
        for (int j = 0; j < TQ_DIM_HI; j++) {
            float xn = hi_rot[j] * inv_hi;
            int idx = nearest(xn, centroids_32_d32, 32);
            pk5(y[i].qs_hi, j, idx);
        }

        // 3-bit MSE for lo (8 centroids, d=32)
        memset(y[i].qs_lo, 0, sizeof(y[i].qs_lo));
        for (int j = 0; j < TQ_DIM_LO; j++) {
            float xn = lo_rot[j] * inv_lo;
            int idx = nearest(xn, centroids_8_d32, 8);
...
void ggml_vec_dot_tqk_6hi_3lo_had_f32(
        int n, float * GGML_RESTRICT s, size_t bs,
        const void * GGML_RESTRICT vx, size_t bx,
        const void * GGML_RESTRICT vy, size_t by, int nrc) {
    assert(n % TQK_BLOCK_SIZE == 0);
    ...
    for (int64_t i = 0; i < nb; i++) {
        ...
        float mse_dot_hi = 0.0f, mse_dot_lo = 0.0f;
        for (int j = 0; j < TQ_DIM_HI; j++) {
            mse_dot_hi += q_rot_hi[j] * centroids_32_d32[up5(x[i].qs_hi, j)];
        }
        for (int j = 0; j < TQ_DIM_LO; j++) {
            mse_dot_lo += q_rot_lo[j] * centroids_8_d32[up3(x[i].qs_lo, j)];
        }

...
constexpr constant static float tq_c32_d32[32] = {
...
// d=256 centroids
constexpr constant static float tq_c16_d256[16] = {
...
constexpr constant static float tq_c8_d192[8] = {
...
constexpr constant static float tq_c2_d96[2] = {
    -0.0816460916f, 0.0816460916f,
};
// d=64, 5-bit
constexpr constant static float tq_c32_d64[32] = {
...
// d=64, 3-bit
constexpr constant static float tq_c8_d64[8] = {
    -0.2639139308f, -0.1661678589f, -0.0938322632f, -0.0304691789f,
     0.0304691789f,  0.0938322632f,  0.1661678589f,  0.2639139308f,
};
// d=64, 2-bit
constexpr constant static float tq_c4_d64[4] = {
    -0.1874968494f, -0.0565148688f,  0.0565148688f,  0.1874968494f,
};
// d=192, 2-bit
constexpr constant static float tq_c4_d192[4] = {
    -0.1087535813f, -0.0326609223f,  0.0326609223f,  0.1087535813f,
};
// d=192, 1-bit
constexpr constant static float tq_c2_d192[2] = {
    -0.0576573838f,  0.0576573838f,
};

// ---------------------------------------------------------------------------
// Structured rotation for non-power-of-2 lo-blocks (96=3×32, 192=3×64)
// R = (I_3 ⊗ H_block) · P · (I_block ⊗ O_3) · D
// ---------------------------------------------------------------------------

constant float tq_O3[9] = {
...
inline float tq_struct_sign(int i, int n) {
    ulong seed = (n == 96) ? 0x5452534C4F393600ULL : 0x5452534C31393200ULL;
    for (int k = 0; k <= i; k++) {
        seed = seed * 6364136223846793005ULL + 1442695040888963407ULL;
    }
    return ((uint(seed >> 32)) & 1) ? -1.0f : 1.0f;
}

template<int N, int BLOCK>
inline void tq_structured_rotate_lo(thread float * x) {
    for (int i = 0; i < N; i++) x[i] *= tq_struct_sign(i, N);
    for (int j = 0; j < BLOCK; j++) {
        float a = x[j], b = x[BLOCK + j], c = x[2*BLOCK + j];
        x[j]          = tq_O3[0]*a + tq_O3[1]*b + tq_O3[2]*c;
        x[BLOCK + j]  = tq_O3[3]*a + tq_O3[4]*b + tq_O3[5]*c;
        x[2*BLOCK + j]= tq_O3[6]*a + tq_O3[7]*b + tq_O3[8]*c;
    }
    float tmp[N];
    for (int i = 0; i < N; i++) tmp[(35*i + 17) % N] = x[i];
    for (int i = 0; i < N; i++) x[i] = tmp[i];
    tq_fwht<BLOCK>(x);
    tq_fwht<BLOCK>(x + BLOCK);
    tq_fwht<BLOCK>(x + 2*BLOCK);
}

template<int N, int BLOCK>
inline void tq_structured_unrotate_lo(thread float * x) {
    tq_fwht<BLOCK>(x);
    tq_fwht<BLOCK>(x + BLOCK);
    tq_fwht<BLOCK>(x + 2*BLOCK);
    float tmp[N];
    ...
    for (int j = 0; j < BLOCK; j++) {
        float a = x[j], b = x[BLOCK + j], c = x[2*BLOCK + j];
        x[j]          = tq_O3[0]*a + tq_O3[3]*b + tq_O3[6]*c;
        x[BLOCK + j]  = tq_O3[1]*a + tq_O3[4]*b + tq_O3[7]*c;
        x[2*BLOCK + j]= tq_O3[2]*a + tq_O3[5]*b + tq_O3[8]*c;
    }
    for (int i = 0; i < N; i++) x[i] *= tq_struct_sign(i, N);
}
...
void dequantize_5hi_3lo_had(device const block_tqk_5hi_3lo * xb, short il, thread type4x4 & reg) {
    ...
    for (int i = 0; i < 16; i++) {
        int j = il * 16 + i;
        if (j < 32) {
            // Hi: 4-bit MSE d32 + Gaussian QJL
            float norm_hi = float(xb->norm_hi);
            float qjl = 1.2533141f / 32.0f * float(xb->rnorm_hi);
            float sign = ((xb->signs_hi[j / 8] >> (j % 8)) & 1) ? 1.0f : -1.0f;
            reg_f[i/4][i%4] = norm_hi * tq_c16_d32[tq_up4(xb->qs_hi, j)] + qjl * sign;
        } else {
            // Lo: 3-bit MSE d32 only (no QJL on lo for _sj)
...
void dequantize_5hi_3lo_had_t4(device const block_tqk_5hi_3lo * xb, short il, thread type4 & reg) {
    for (int i = 0; i < 4; i++) {
        ...
        if (j < 32) {
            ...
            float qjl = 1.2533141f / 32.0f * float(xb->rnorm_hi);
            float sign = ((xb->signs_hi[j / 8] >> (j % 8)) & 1) ? 1.0f : -1.0f;
            reg[i] = norm_hi * tq_c16_d32[tq_up4(xb->qs_hi, j)] + qjl * sign;
        } else {
            float norm_lo = float(xb->norm_lo);
            int lo_j = j - 32;
...
// 6hi_3lo_had (tqk4_sj): 5-bit MSE on hi (32 centroids) + QJL, 3-bit MSE on lo (no QJL)
template <typename type4x4>
void dequantize_6hi_3lo_had(device const block_tqk_6hi_3lo * xb, short il, thread type4x4 & reg) {
    float4x4 reg_f;
    for (int i = 0; i < 16; i++) {
        int j = il * 16 + i;
        if (j < 32) {
...
template <typename type4>
void dequantize_6hi_3lo_had_t4(device const block_tqk_6hi_3lo * xb, short il, thread type4 & reg) {
    for (int i = 0; i < 4; i++) {
        int j = il * 4 + i;
        if (j < 32) {
            float norm_hi = float(xb->norm_hi);
...
// 5r3_sj (tqk5r3_sj): 5-bit MSE + 3-bit residual on hi, 3-bit MSE on lo (no QJL)
template <typename type4x4>
void dequantize_5r3_sj(device const block_tqk_5r3_sj * xb, short il, thread type4x4 & reg) {
    float4x4 reg_f;
    for (int i = 0; i < 16; i++) {
        int j = il * 16 + i;
        if (j < 32) {
            float norm_hi = float(xb->norm_hi);
            float rnorm2  = float(xb->rnorm2_hi);
            int v1 = tq_up5(xb->qs_hi, j);
            int v2 = tq_up3(xb->qs_hi2, j);
            reg_f[i/4][i%4] = norm_hi * tq_c32_d32[v1] + rnorm2 * tq_c8_d32[v2];
        } else {
            float norm_lo = float(xb->norm_lo);
            int lo_j = j - 32;
            reg_f[i/4][i%4] = norm_lo * tq_c8_d96[tq_up3(xb->qs_lo, lo_j)];
...
template <typename type4>
void dequantize_5r3_sj_t4(device const block_tqk_5r3_sj * xb, short il, thread type4 & reg) {
    for (int i = 0; i < 4; i++) {
        int j = il * 4 + i;
        if (j < 32) {
            float norm_hi = float(xb->norm_hi);
            ...
            int v1 = tq_up5(xb->qs_hi, j);
            int v2 = tq_up3(xb->qs_hi2, j);
            reg[i] = norm_hi * tq_c32_d32[v1] + rnorm2 * tq_c8_d32[v2];
        } else {
            float norm_lo = float(xb->norm_lo);
            int lo_j = j - 32;
            reg[i] = norm_lo * tq_c8_d96[tq_up3(xb->qs_lo, lo_j)];
...
// 6hi_3lo_jj_had (tqk4_sjj): 5-bit MSE + QJL on hi, 3-bit MSE + QJL on lo
template <typename type4x4>
void dequantize_6hi_3lo_jj_had(device const block_tqk_6hi_3lo_jj * xb, short il, thread type4x4 & reg) {
    float4x4 reg_f;
    for (int i = 0; i < 16; i++) {
        int j = il * 16 + i;
        if (j < 32) {
...
template <typename type4>
void dequantize_6hi_3lo_jj_had_t4(device const block_tqk_6hi_3lo_jj * xb, short il, thread type4 & reg) {
    for (int i = 0; i < 4; i++) {
        int j = il * 4 + i;
        if (j < 32) {
            float norm_hi = float(xb->norm_hi);
...
// ---- d=256 TurboQuant dequantize functions ----

// 4-bit MSE only (had_mse4_d256)
template <typename type4x4>
void dequantize_had_mse4_d256(device const block_tqk_had_mse4_d256 * xb, short il, thread type4x4 & reg) {
    float norm = float(xb->norm);
    float4x4 reg_f;
    for (int i = 0; i < 16; i++) {
        int j = il * 16 + i;
        reg_f[i/4][i%4] = norm * tq_c16_d256[tq_up4(xb->qs, j)];
    }
    reg = (type4x4) reg_f;
}
template <typename type4>
void dequantize_had_mse4_d256_t4(device const block_tqk_had_mse4_d256 * xb, short il, thread type4 & reg) {
    float norm = float(xb->norm);
    for (int i = 0; i < 4; i++) {
        int j = il * 4 + i;
        reg[i] = norm * tq_c16_d256[tq_up4(xb->qs, j)];
    }
}

// 4-bit MSE + 1-bit QJL (had_prod5_d256)
template <typename type4x4>
void dequantize_had_prod5_d256(device const block_tqk_had_prod5_d256 * xb, short il, thread type4x4 & reg) {
    float norm = float(xb->norm);
    float qjl = 1.2533141f / 256.0f * float(xb->rnorm);
    float4x4 reg_f;
    for (int i = 0; i < 16; i++) {
        int j = il * 16 + i;
        float sign = ((xb->signs[j / 8] >> (j % 8)) & 1) ? 1.0f : -1.0f;
        reg_f[i/4][i%4] = norm * tq_c16_d256[tq_up4(xb->qs, j)] + qjl * sign;
    }
    reg = (type4x4) reg_f;
}
template <typename type4>
void dequantize_had_prod5_d256_t4(device const block_tqk_had_prod5_d256 * xb, short il, thread type4 & reg) {
    float norm = float(xb->norm);
    float qjl = 1.2533141f / 256.0f * float(xb->rnorm);
    for (int i = 0; i < 4; i++) {
        int j = il * 4 + i;
        float sign = ((xb->signs[j / 8] >> (j % 8)) & 1) ? 1.0f : -1.0f;
        reg[i] = norm * tq_c16_d256[tq_up4(xb->qs, j)] + qjl * sign;
    }
}

// 3-bit MSE + 1-bit QJL (had_prod4_d256)
template <typename type4x4>
void dequantize_had_prod4_d256(device const block_tqk_had_prod4_d256 * xb, short il, thread type4x4 & reg) {
    float norm = float(xb->norm);
    float qjl = 1.2533141f / 256.0f * float(xb->rnorm);
    float4x4 reg_f;
    for (int i = 0; i < 16; i++) {
        int j = il * 16 + i;
        float sign = ((xb->signs[j / 8] >> (j % 8)) & 1) ? 1.0f : -1.0f;
        reg_f[i/4][i%4] = norm * tq_c8_d256[tq_up3(xb->qs, j)] + qjl * sign;
    }
    reg = (type4x4) reg_f;
}
template <typename type4>
void dequantize_had_prod4_d256_t4(device const block_tqk_had_prod4_d256 * xb, short il, thread type4 & reg) {
    float norm = float(xb->norm);
    float qjl = 1.2533141f / 256.0f * float(xb->rnorm);
    for (int i = 0; i < 4; i++) {
        int j = il * 4 + i;
        float sign = ((xb->signs[j / 8] >> (j % 8)) & 1) ? 1.0f : -1.0f;
        reg[i] = norm * tq_c8_d256[tq_up3(xb->qs, j)] + qjl * sign;
    }
}

// 5hi_3lo_had_d256 (tqk3_sj_d256): FA dequantize — outputs [hi(64), lo(192)] in block order
// Elements 0-63: 4-bit MSE d64 + QJL (hi/outlier channels)
// Elements 64-255: 3-bit MSE d64 (lo/regular channels, 3 blocks of 64) — no QJL on lo
template <typename type4x4>
void dequantize_5hi_3lo_had_d256(device const block_tqk_5hi_3lo_d256 * xb, short il, thread type4x4 & reg) {
    float4x4 reg_f;
    for (int i = 0; i < 16; i++) {
        int j = il * 16 + i;
        if (j < 64) {
            // Hi: 4-bit MSE d64 + Gaussian QJL
            float norm_hi = float(xb->norm_hi);
            float qjl = 1.2533141f / 64.0f * float(xb->rnorm_hi);
            float sign = ((xb->signs_hi[j / 8] >> (j % 8)) & 1) ? 1.0f : -1.0f;
            reg_f[i/4][i%4] = norm_hi * tq_c16_d64[tq_up4(xb->qs_hi, j)] + qjl * sign;
        } else {
            // Lo: 3-bit MSE d64 only (no QJL on lo for _sj)
            float norm_lo = float(xb->norm_lo);
            int lo_j = j - 64;
            reg_f[i/4][i%4] = norm_lo * tq_c8_d192[tq_up3(xb->qs_lo, lo_j)];
        }
    }
    reg = (type4x4) reg_f;
}
template <typename type4>
void dequantize_5hi_3lo_had_d256_t4(device const block_tqk_5hi_3lo_d256 * xb, short il, thread type4 & reg) {
    for (int i = 0; i < 4; i++) {
        int j = il * 4 + i;
        if (j < 64) {
            float norm_hi = float(xb->norm_hi);
            float qjl = 1.2533141f / 64.0f * float(xb->rnorm_hi);
            float sign = ((xb->signs_hi[j / 8] >> (j % 8)) & 1) ? 1.0f : -1.0f;
            reg[i] = norm_hi * tq_c16_d64[tq_up4(xb->qs_hi, j)] + qjl * sign;
        } else {
            float norm_lo = float(xb->norm_lo);
            int lo_j = j - 64;
            reg[i] = norm_lo * tq_c8_d192[tq_up3(xb->qs_lo, lo_j)];
        }
    }
}

// 6hi_3lo_had_d256 (tqk4_sj_d256): FA dequantize — 5-bit MSE + QJL on hi(64), 3-bit MSE on lo(192), no QJL on lo
template <typename type4x4>
void dequantize_6hi_3lo_had_d256(device const block_tqk_6hi_3lo_d256 * xb, short il, thread type4x4 & reg) {
    float4x4 reg_f;
    for (int i = 0; i < 16; i++) {
        int j = il * 16 + i;
        if (j < 64) {
...
template <typename type4>
void dequantize_6hi_3lo_had_d256_t4(device const block_tqk_6hi_3lo_d256 * xb, short il, thread type4 & reg) {
    for (int i = 0; i < 4; i++) {
        int j = il * 4 + i;
        if (j < 64) {
            float norm_hi = float(xb->norm_hi);
...
// 6hi_3lo_jj_had_d256 (tqk4_sjj_d256): FA dequantize — 5-bit MSE + QJL on hi(64), 3-bit MSE + QJL on lo(192)
template <typename type4x4>
void dequantize_6hi_3lo_jj_had_d256(device const block_tqk_6hi_3lo_jj_d256 * xb, short il, thread type4x4 & reg) {
    float4x4 reg_f;
    for (int i = 0; i < 16; i++) {
        int j = il * 16 + i;
        if (j < 64) {
...
template <typename type4>
void dequantize_6hi_3lo_jj_had_d256_t4(device const block_tqk_6hi_3lo_jj_d256 * xb, short il, thread type4 & reg) {
    for (int i = 0; i < 4; i++) {
        int j = il * 4 + i;
        if (j < 64) {
            float norm_hi = float(xb->norm_hi);
...
// 2hi_1lo_had_d256: FA dequantize — 2-bit MSE + QJL on hi(64), 1-bit MSE + QJL on lo(192)
template <typename type4x4>
void dequantize_2hi_1lo_had_d256(device const block_tqk_2hi_1lo_d256 * xb, short il, thread type4x4 & reg) {
    float4x4 reg_f;
    for (int i = 0; i < 16; i++) {
        int j = il * 16 + i;
        if (j < 64) {
...
template <typename type4>
void dequantize_2hi_1lo_had_d256_t4(device const block_tqk_2hi_1lo_d256 * xb, short il, thread type4 & reg) {
    for (int i = 0; i < 4; i++) {
        int j = il * 4 + i;
        if (j < 64) {
            float norm_hi = float(xb->norm_hi);
...
// 3hi_2lo_had_d256: FA dequantize — 3-bit MSE + QJL on hi(64), 2-bit MSE + QJL on lo(192)
template <typename type4x4>
void dequantize_3hi_2lo_had_d256(device const block_tqk_3hi_2lo_d256 * xb, short il, thread type4x4 & reg) {
    float4x4 reg_f;
    for (int i = 0; i < 16; i++) {
        int j = il * 16 + i;
        if (j < 64) {
...
template <typename type4>
void dequantize_3hi_2lo_had_d256_t4(device const block_tqk_3hi_2lo_d256 * xb, short il, thread type4 & reg) {
    for (int i = 0; i < 4; i++) {
        int j = il * 4 + i;
        if (j < 64) {
            float norm_hi = float(xb->norm_hi);
...
template<
    ...
    bool TQ_SPLIT_STRUCT = false, // structured rotation for _sjj split types (sign+O3+perm+FWHT)
...
void kernel_flash_attn_ext_impl(
        ...
        device       char * dst,
        device const char * tq_chmap, // per-head channel permutation [n_kv_heads][DK] (TQ_SPLIT only)
        threadgroup  half * shmem_f16,
        threadgroup  uint8_t * tq_perm, // DK-byte scratch for channel permutation (TQ_SPLIT only)
        threadgroup  half * tq_rot_tmp, // per-simd-group scratch for structured rotation permutation
        uint3   tgpig,
        ushort  tiisg,
        ushort  sgitg) {
...
#define NS20 (FC_flash_attn_ext_ns20)
    ...
    short ikv2;
    {
        ...
        ikv2 = iq2/(args.ne02/args.ne_12_2);
    ...
    // TQ_SPLIT: load channel permutation for current KV head
    if (TQ_SPLIT) {
        device const uint8_t * perm_src = (device const uint8_t *)tq_chmap + ikv2 * DK;
        for (short i = tiisg + sgitg*NW; i < DK; i += NW*NSG) {
            tq_perm[i] = perm_src[i];
        }
        threadgroup_barrier(mem_flags::mem_threadgroup);
    }
    ...
    if (TQ_SPLIT) {
        // Permuted Q load: rearrange channels to [outlier, regular] order
        FOR_UNROLL (short jj = 0; jj < NQ; ++jj) {
            const short j = jj*NSG + sgitg;
            device const float * q_head = (device const float *) ((device const char *) q + j*args.nb01);
            threadgroup half * sq_j = (threadgroup half *)(sq + j*DK);
            for (short i = tiisg; i < DK; i += NW) {
                if (iq1 + j < args.ne01) {
                    sq_j[i] = half(q_head[tq_perm[i]]);
                } else {
                    sq_j[i] = 0;
                }
            }
        }
        // Apply rotation: FWHT on hi, rotation on lo
        // TQ_SPLIT_STRUCT: full structured rotation (sign+O3+perm+FWHT) for _sjj
        // !TQ_SPLIT_STRUCT: bare 3×FWHT for _sj
        // Serialized across simd groups (shared scratch tq_rot_tmp)
        for (short sg = 0; sg < NSG; ++sg) {
            FOR_UNROLL (short jj = 0; jj < NQ; ++jj) {
                const short j = jj*NSG + sg;
                if (sgitg == sg && iq1 + j < args.ne01) {
                    threadgroup half * sq_j = (threadgroup half *)(sq + j*DK);
                    constexpr short lo_off = DK/4;
                    constexpr short lo_n = DK - lo_off;
                    constexpr short blk_dim = lo_n / 3;

                    // Hi block: plain FWHT
                    tq_fwht_shared<DK/4>(sq_j, tiisg);

                    if (TQ_SPLIT_STRUCT) {
                        // Lo: steps 1+2 fused (sign + O_3 mix) → scratch
                        for (short col = tiisg; col < blk_dim; col += NW) {
                            float a = float(sq_j[lo_off + col])             * tq_struct_sign(col, lo_n);
                            float b = float(sq_j[lo_off + blk_dim + col])   * tq_struct_sign(blk_dim + col, lo_n);
                            float c = float(sq_j[lo_off + 2*blk_dim + col]) * tq_struct_sign(2*blk_dim + col, lo_n);
                            tq_rot_tmp[col]             = half(tq_O3[0]*a + tq_O3[1]*b + tq_O3[2]*c);
                            tq_rot_tmp[blk_dim + col]   = half(tq_O3[3]*a + tq_O3[4]*b + tq_O3[5]*c);
                            tq_rot_tmp[2*blk_dim + col] = half(tq_O3[6]*a + tq_O3[7]*b + tq_O3[8]*c);
                        }
                        simdgroup_barrier(mem_flags::mem_threadgroup);

                        // Lo: step 3 — permutation
                        for (short i = tiisg; i < lo_n; i += NW) {
                            sq_j[lo_off + (35*i + 17) % lo_n] = tq_rot_tmp[i];
                        }
                        simdgroup_barrier(mem_flags::mem_threadgroup);
                    }

                    // Lo: 3 × FWHT (step 4 of structured, or sole step for bare)
                    for (short blki = 0; blki < lo_n; blki += blk_dim) {
                        tq_fwht_shared<DK/4>(sq_j + lo_off + blki, tiisg);
                    }
                }
            }
            threadgroup_barrier(mem_flags::mem_threadgroup);
        ...
        // pre-rotate Q with FWHT_128 for had_mse4/prod5/prod4
        if (TQ_FWHT) {
            FOR_UNROLL (short jj = 0; jj < NQ; ++jj) {
                const short j = jj*NSG + sgitg;
                if (iq1 + j < args.ne01) {
...
template<
    ...
    bool TQ_SPLIT_STRUCT = false, // structured rotation for _sjj split types
    bool TQ_FWHT_O = false> // apply inverse FWHT_128 to output (V stored rotated)
kernel void kernel_flash_attn_ext(
        ...
        device const char * blk,
        device       char * dst,
        device const char * tq_chmap,
        threadgroup  half * shmem_f16 [[threadgroup(0)]],
        uint3   tgpig[[threadgroup_position_in_grid]],
        ushort  tiisg[[thread_index_in_simdgroup]],
        ushort  sgitg[[simdgroup_index_in_threadgroup]]) {
    threadgroup uint8_t tq_perm_buf[DK];
    threadgroup half tq_rot_buf[DK]; // scratch for structured rotation permutation step
...
#define FWD_ARGS args, q, k, v, mask, sinks, pad, blk, dst, tq_chmap, shmem_f16, tq_perm_buf, tq_rot_buf, tgpig, tiisg, sgitg
    switch (FC_flash_attn_ext_nsg) {
      // note: disabled cases to reduce library load time
      //case 1: kernel_flash_attn_ext_impl<FWD_TMPL, 1, TQ_FWHT, TQ_SPLIT, TQ_SPLIT_STRUCT, TQ_FWHT_O>(FWD_ARGS); break;
      //case 2: kernel_flash_attn_ext_impl<FWD_TMPL, 2, TQ_FWHT, TQ_SPLIT, TQ_SPLIT_STRUCT, TQ_FWHT_O>(FWD_ARGS); break;
        case 4: kernel_flash_attn_ext_impl<FWD_TMPL, 4, TQ_FWHT, TQ_SPLIT, TQ_SPLIT_STRUCT, TQ_FWHT_O>(FWD_ARGS); break;
        case 8: kernel_flash_attn_ext_impl<FWD_TMPL, 8, TQ_FWHT, TQ_SPLIT, TQ_SPLIT_STRUCT, TQ_FWHT_O>(FWD_ARGS); break;
    }
#undef FWD_TMPL
#undef FWD_ARGS
}
...
template [[host_name("kernel_flash_attn_ext_tqk4_0_f16_dk128_dv128")]]  kernel flash_attn_ext_t kernel_flash_attn_ext<FA_TYPES, block_tqk_had_mse4,  8, dequantize_had_mse4,  half4x4, 1, dequantize_f16, 128, 128, OP_FLASH_ATTN_EXT_NQPSG, OP_FLASH_ATTN_EXT_NCPSG, true>;
template [[host_name("kernel_flash_attn_ext_tqk5_0j_f16_dk128_dv128")]] kernel flash_attn_ext_t kernel_flash_attn_ext<FA_TYPES, block_tqk_had_prod5, 8, dequantize_had_prod5, half4x4, 1, dequantize_f16, 128, 128, OP_FLASH_ATTN_EXT_NQPSG, OP_FLASH_ATTN_EXT_NCPSG, true>;
template [[host_name("kernel_flash_attn_ext_tqk4_1j_f16_dk128_dv128")]] kernel flash_attn_ext_t kernel_flash_attn_ext<FA_TYPES, block_tqk_had_prod4, 8, dequantize_had_prod4, half4x4, 1, dequantize_f16, 128, 128, OP_FLASH_ATTN_EXT_NQPSG, OP_FLASH_ATTN_EXT_NCPSG, true>;
// 5hi_3lo_had K (split channels, Q permuted + 4×FWHT_32) + f16 V
template [[host_name("kernel_flash_attn_ext_tqk3_sj_f16_dk128_dv128")]] kernel flash_attn_ext_t kernel_flash_attn_ext<FA_TYPES, block_tqk_5hi_3lo, 8, dequantize_5hi_3lo_had, half4x4, 1, dequantize_f16, 128, 128, OP_FLASH_ATTN_EXT_NQPSG, OP_FLASH_ATTN_EXT_NCPSG, false, true>;
// TQ K + TQV V (V stored rotated, inverse FWHT on output)
template [[host_name("kernel_flash_attn_ext_tqk4_0_tqv4_0_dk128_dv128")]]  kernel flash_attn_ext_t kernel_flash_attn_ext<FA_TYPES, block_tqk_had_mse4,  8, dequantize_had_mse4,  block_tqv_had_mse4, 8, dequantize_had_mse4, 128, 128, OP_FLASH_ATTN_EXT_NQPSG, OP_FLASH_ATTN_EXT_NCPSG, true,  false, false, true>;
template [[host_name("kernel_flash_attn_ext_tqk5_0j_tqv4_0_dk128_dv128")]] kernel flash_attn_ext_t kernel_flash_attn_ext<FA_TYPES, block_tqk_had_prod5, 8, dequantize_had_prod5, block_tqv_had_mse4, 8, dequantize_had_mse4, 128, 128, OP_FLASH_ATTN_EXT_NQPSG, OP_FLASH_ATTN_EXT_NCPSG, true,  false, false, true>;
template [[host_name("kernel_flash_attn_ext_tqk4_1j_tqv4_0_dk128_dv128")]] kernel flash_attn_ext_t kernel_flash_attn_ext<FA_TYPES, block_tqk_had_prod4, 8, dequantize_had_prod4, block_tqv_had_mse4, 8, dequantize_had_mse4, 128, 128, OP_FLASH_ATTN_EXT_NQPSG, OP_FLASH_ATTN_EXT_NCPSG, true,  false, false, true>;
template [[host_name("kernel_flash_attn_ext_tqk3_sj_tqv4_0_dk128_dv128")]] kernel flash_attn_ext_t kernel_flash_attn_ext<FA_TYPES, block_tqk_5hi_3lo, 8, dequantize_5hi_3lo_had, block_tqv_had_mse4, 8, dequantize_had_mse4, 128, 128, OP_FLASH_ATTN_EXT_NQPSG, OP_FLASH_ATTN_EXT_NCPSG, false, true, false, true>;
// f16 K + TQV V (V stored rotated, inverse FWHT on output)
template [[host_name("kernel_flash_attn_ext_f16_tqv4_0_dk128_dv128")]] kernel flash_attn_ext_t kernel_flash_attn_ext<FA_TYPES, half4x4, 1, dequantize_f16, block_tqv_had_mse4, 8, dequantize_had_mse4, 128, 128, OP_FLASH_ATTN_EXT_NQPSG, OP_FLASH_ATTN_EXT_NCPSG, false, false, false, true>;
// TQ K + q4_0 V
template [[host_name("kernel_flash_attn_ext_tqk4_0_q4_0_dk128_dv128")]]  kernel flash_attn_ext_t kernel_flash_attn_ext<FA_TYPES, block_tqk_had_mse4,  8, dequantize_had_mse4,  block_q4_0, 2, dequantize_q4_0, 128, 128, OP_FLASH_ATTN_EXT_NQPSG, OP_FLASH_ATTN_EXT_NCPSG, true>;
template [[host_name("kernel_flash_attn_ext_tqk5_0j_q4_0_dk128_dv128")]] kernel flash_attn_ext_t kernel_flash_attn_ext<FA_TYPES, block_tqk_had_prod5, 8, dequantize_had_prod5, block_q4_0, 2, dequantize_q4_0, 128, 128, OP_FLASH_ATTN_EXT_NQPSG, OP_FLASH_ATTN_EXT_NCPSG, true>;
template [[host_name("kernel_flash_attn_ext_tqk4_1j_q4_0_dk128_dv128")]] kernel flash_attn_ext_t kernel_flash_attn_ext<FA_TYPES, block_tqk_had_prod4, 8, dequantize_had_prod4, block_q4_0, 2, dequantize_q4_0, 128, 128, OP_FLASH_ATTN_EXT_NQPSG, OP_FLASH_ATTN_EXT_NCPSG, true>;
template [[host_name("kernel_flash_attn_ext_tqk3_sj_q4_0_dk128_dv128")]] kernel flash_attn_ext_t kernel_flash_attn_ext<FA_TYPES, block_tqk_5hi_3lo, 8, dequantize_5hi_3lo_had, block_q4_0, 2, dequantize_q4_0, 128, 128, OP_FLASH_ATTN_EXT_NQPSG, OP_FLASH_ATTN_EXT_NCPSG, false, true>;

// TQK 6hi_3lo (tqk4_sj) FA
...
template [[host_name("kernel_flash_attn_ext_tqk4_sj_tqv4_0_dk128_dv128")]]     kernel flash_attn_ext_t kernel_flash_attn_ext<FA_TYPES, block_tqk_6hi_3lo, 8, dequantize_6hi_3lo_had, block_tqv_had_mse4, 8, dequantize_had_mse4, 128, 128, OP_FLASH_ATTN_EXT_NQPSG, OP_FLASH_ATTN_EXT_NCPSG, false, true, false, true>;
...
// TQK 5r3_sj (tqk5r3_sj) FA — 5-bit+3-bit residual hi, 3-bit lo, no QJL
template [[host_name("kernel_flash_attn_ext_tqk5r3_sj_f16_dk128_dv128")]]      kernel flash_attn_ext_t kernel_flash_attn_ext<FA_TYPES, block_tqk_5r3_sj, 8, dequantize_5r3_sj, half4x4, 1, dequantize_f16, 128, 128, OP_FLASH_ATTN_EXT_NQPSG, OP_FLASH_ATTN_EXT_NCPSG, false, true>;
template [[host_name("kernel_flash_attn_ext_tqk5r3_sj_tqv4_0_dk128_dv128")]]   kernel flash_attn_ext_t kernel_flash_attn_ext<FA_TYPES, block_tqk_5r3_sj, 8, dequantize_5r3_sj, block_tqv_had_mse4, 8, dequantize_had_mse4, 128, 128, OP_FLASH_ATTN_EXT_NQPSG, OP_FLASH_ATTN_EXT_NCPSG, false, true, false, true>;
template [[host_name("kernel_flash_attn_ext_tqk5r3_sj_q4_0_dk128_dv128")]]     kernel flash_attn_ext_t kernel_flash_attn_ext<FA_TYPES, block_tqk_5r3_sj, 8, dequantize_5r3_sj, block_q4_0, 2, dequantize_q4_0, 128, 128, OP_FLASH_ATTN_EXT_NQPSG, OP_FLASH_ATTN_EXT_NCPSG, false, true>;
// TQK 6hi_3lo_jj (tqk4_sjj) FA — uses block_tqk_6hi_3lo_jj with QJL on both hi and lo
template [[host_name("kernel_flash_attn_ext_tqk4_sjj_f16_dk128_dv128")]]       kernel flash_attn_ext_t kernel_flash_attn_ext<FA_TYPES, block_tqk_6hi_3lo_jj, 8, dequantize_6hi_3lo_jj_had, half4x4, 1, dequantize_f16, 128, 128, OP_FLASH_ATTN_EXT_NQPSG, OP_FLASH_ATTN_EXT_NCPSG, false, true, true>;
template [[host_name("kernel_flash_attn_ext_tqk4_sjj_tqv4_0_dk128_dv128")]]    kernel flash_attn_ext_t kernel_flash_attn_ext<FA_TYPES, block_tqk_6hi_3lo_jj, 8, dequantize_6hi_3lo_jj_had, block_tqv_had_mse4, 8, dequantize_had_mse4, 128, 128, OP_FLASH_ATTN_EXT_NQPSG, OP_FLASH_ATTN_EXT_NCPSG, false, true, true, true>;
template [[host_name("kernel_flash_attn_ext_tqk4_sjj_q4_0_dk128_dv128")]]      kernel flash_attn_ext_t kernel_flash_attn_ext<FA_TYPES, block_tqk_6hi_3lo_jj, 8, dequantize_6hi_3lo_jj_had, block_q4_0, 2, dequantize_q4_0, 128, 128, OP_FLASH_ATTN_EXT_NQPSG, OP_FLASH_ATTN_EXT_NCPSG, false, true, true>;
...
template [[host_name("kernel_flash_attn_ext_tqk2_sjj_tqv4_0_dk128_dv128")]]     kernel flash_attn_ext_t kernel_flash_attn_ext<FA_TYPES, block_tqk_2hi_1lo, 8, dequantize_2hi_1lo_had, block_tqv_had_mse4, 8, dequantize_had_mse4, 128, 128, OP_FLASH_ATTN_EXT_NQPSG, OP_FLASH_ATTN_EXT_NCPSG, false, true, true, true>;
template [[host_name("kernel_flash_attn_ext_tqk2_sjj_q4_0_dk128_dv128")]]       kernel flash_attn_ext_t kernel_flash_attn_ext<FA_TYPES, block_tqk_2hi_1lo, 8, dequantize_2hi_1lo_had, block_q4_0, 2, dequantize_q4_0, 128, 128, OP_FLASH_ATTN_EXT_NQPSG, OP_FLASH_ATTN_EXT_NCPSG, false, true, true>;
// TQK 3hi_2lo (tqk3_sjj) FA
template [[host_name("kernel_flash_attn_ext_tqk3_sjj_f16_dk128_dv128")]]       kernel flash_attn_ext_t kernel_flash_attn_ext<FA_TYPES, block_tqk_3hi_2lo, 8, dequantize_3hi_2lo_had, half4x4, 1, dequantize_f16, 128, 128, OP_FLASH_ATTN_EXT_NQPSG, OP_FLASH_ATTN_EXT_NCPSG, false, true, true>;
template [[host_name("kernel_flash_attn_ext_tqk3_sjj_tqv4_0_dk128_dv128")]]    kernel flash_attn_ext_t kernel_flash_attn_ext<FA_TYPES, block_tqk_3hi_2lo, 8, dequantize_3hi_2lo_had, block_tqv_had_mse4, 8, dequantize_had_mse4, 128, 128, OP_FLASH_ATTN_EXT_NQPSG, OP_FLASH_ATTN_EXT_NCPSG, false, true, true, true>;
template [[host_name("kernel_flash_attn_ext_tqk3_sjj_q4_0_dk128_dv128")]]      kernel flash_attn_ext_t kernel_flash_attn_ext<FA_TYPES, block_tqk_3hi_2lo, 8, dequantize_3hi_2lo_had, block_q4_0, 2, dequantize_q4_0, 128, 128, OP_FLASH_ATTN_EXT_NQPSG, OP_FLASH_ATTN_EXT_NCPSG, false, true, true>;
...
template [[host_name("kernel_flash_attn_ext_tqk4_1j_d256_f16_dk256_dv256")]] kernel flash_attn_ext_t kernel_flash_attn_ext<FA_TYPES, block_tqk_had_prod4_d256, 16, dequantize_had_prod4_d256, half4x4, 1, dequantize_f16, 256, 256, OP_FLASH_ATTN_EXT_NQPSG, OP_FLASH_ATTN_EXT_NCPSG, true>;
// 5hi_3lo_had d=256 K + f16 V
template [[host_name("kernel_flash_attn_ext_tqk3_sj_d256_f16_dk256_dv256")]] kernel flash_attn_ext_t kernel_flash_attn_ext<FA_TYPES, block_tqk_5hi_3lo_d256, 16, dequantize_5hi_3lo_had_d256, half4x4, 1, dequantize_f16, 256, 256, OP_FLASH_ATTN_EXT_NQPSG, OP_FLASH_ATTN_EXT_NCPSG, false, true>;
// f16 K + TQV d=256 V
template [[host_name("kernel_flash_attn_ext_f16_tqv4_0_d256_dk256_dv256")]] kernel flash_attn_ext_t kernel_flash_attn_ext<FA_TYPES, half4x4, 1, dequantize_f16, block_tqv_had_mse4_d256, 16, dequantize_had_mse4_d256, 256, 256, OP_FLASH_ATTN_EXT_NQPSG, OP_FLASH_ATTN_EXT_NCPSG, false, false, false, true>;
// TQ d=256 K + TQV d=256 V
template [[host_name("kernel_flash_attn_ext_tqk4_0_d256_tqv4_0_d256_dk256_dv256")]]  kernel flash_attn_ext_t kernel_flash_attn_ext<FA_TYPES, block_tqk_had_mse4_d256,  16, dequantize_had_mse4_d256,  block_tqv_had_mse4_d256, 16, dequantize_had_mse4_d256, 256, 256, OP_FLASH_ATTN_EXT_NQPSG, OP_FLASH_ATTN_EXT_NCPSG, true,  false, false, true>;
template [[host_name("kernel_flash_attn_ext_tqk5_0j_d256_tqv4_0_d256_dk256_dv256")]] kernel flash_attn_ext_t kernel_flash_attn_ext<FA_TYPES, block_tqk_had_prod5_d256, 16, dequantize_had_prod5_d256, block_tqv_had_mse4_d256, 16, dequantize_had_mse4_d256, 256, 256, OP_FLASH_ATTN_EXT_NQPSG, OP_FLASH_ATTN_EXT_NCPSG, true,  false, false, true>;
template [[host_name("kernel_flash_attn_ext_tqk4_1j_d256_tqv4_0_d256_dk256_dv256")]] kernel flash_attn_ext_t kernel_flash_attn_ext<FA_TYPES, block_tqk_had_prod4_d256, 16, dequantize_had_prod4_d256, block_tqv_had_mse4_d256, 16, dequantize_had_mse4_d256, 256, 256, OP_FLASH_ATTN_EXT_NQPSG, OP_FLASH_ATTN_EXT_NCPSG, true,  false, false, true>;
template [[host_name("kernel_flash_attn_ext_tqk3_sj_d256_tqv4_0_d256_dk256_dv256")]] kernel flash_attn_ext_t kernel_flash_attn_ext<FA_TYPES, block_tqk_5hi_3lo_d256, 16, dequantize_5hi_3lo_had_d256, block_tqv_had_mse4_d256, 16, dequantize_had_mse4_d256, 256, 256, OP_FLASH_ATTN_EXT_NQPSG, OP_FLASH_ATTN_EXT_NCPSG, false, true, false, true>;
// 6hi_3lo_had d=256 K + f16/tqv V
template [[host_name("kernel_flash_attn_ext_tqk4_sj_d256_f16_dk256_dv256")]]             kernel flash_attn_ext_t kernel_flash_attn_ext<FA_TYPES, block_tqk_6hi_3lo_d256, 16, dequantize_6hi_3lo_had_d256, half4x4, 1, dequantize_f16, 256, 256, OP_FLASH_ATTN_EXT_NQPSG, OP_FLASH_ATTN_EXT_NCPSG, false, true>;
template [[host_name("kernel_flash_attn_ext_tqk4_sj_d256_tqv4_0_d256_dk256_dv256")]]     kernel flash_attn_ext_t kernel_flash_attn_ext<FA_TYPES, block_tqk_6hi_3lo_d256, 16, dequantize_6hi_3lo_had_d256, block_tqv_had_mse4_d256, 16, dequantize_had_mse4_d256, 256, 256, OP_FLASH_ATTN_EXT_NQPSG, OP_FLASH_ATTN_EXT_NCPSG, false, true, false, true>;
// 6hi_3lo_had_jj d=256 (tqk4_sjj) — shares dequant with tqk4_sj
template [[host_name("kernel_flash_attn_ext_tqk4_sjj_d256_f16_dk256_dv256")]]            kernel flash_attn_ext_t kernel_flash_attn_ext<FA_TYPES, block_tqk_6hi_3lo_jj_d256, 16, dequantize_6hi_3lo_jj_had_d256, half4x4, 1, dequantize_f16, 256, 256, OP_FLASH_ATTN_EXT_NQPSG, OP_FLASH_ATTN_EXT_NCPSG, false, true, true>;
template [[host_name("kernel_flash_attn_ext_tqk4_sjj_d256_tqv4_0_d256_dk256_dv256")]]    kernel flash_attn_ext_t kernel_flash_attn_ext<FA_TYPES, block_tqk_6hi_3lo_jj_d256, 16, dequantize_6hi_3lo_jj_had_d256, block_tqv_had_mse4_d256, 16, dequantize_had_mse4_d256, 256, 256, OP_FLASH_ATTN_EXT_NQPSG, OP_FLASH_ATTN_EXT_NCPSG, false, true, true, true>;
// 2hi_1lo_had d=256 K + f16/tqv V
template [[host_name("kernel_flash_attn_ext_tqk2_sjj_d256_f16_dk256_dv256")]]             kernel flash_attn_ext_t kernel_flash_attn_ext<FA_TYPES, block_tqk_2hi_1lo_d256, 16, dequantize_2hi_1lo_had_d256, half4x4, 1, dequantize_f16, 256, 256, OP_FLASH_ATTN_EXT_NQPSG, OP_FLASH_ATTN_EXT_NCPSG, false, true, true>;
template [[host_name("kernel_flash_attn_ext_tqk2_sjj_d256_tqv4_0_d256_dk256_dv256")]]     kernel flash_attn_ext_t kernel_flash_attn_ext<FA_TYPES, block_tqk_2hi_1lo_d256, 16, dequantize_2hi_1lo_had_d256, block_tqv_had_mse4_d256, 16, dequantize_had_mse4_d256, 256, 256, OP_FLASH_ATTN_EXT_NQPSG, OP_FLASH_ATTN_EXT_NCPSG, false, true, true, true>;
// 3hi_2lo_had d=256 K + f16/tqv V
template [[host_name("kernel_flash_attn_ext_tqk3_sjj_d256_f16_dk256_dv256")]]            kernel flash_attn_ext_t kernel_flash_attn_ext<FA_TYPES, block_tqk_3hi_2lo_d256, 16, dequantize_3hi_2lo_had_d256, half4x4, 1, dequantize_f16, 256, 256, OP_FLASH_ATTN_EXT_NQPSG, OP_FLASH_ATTN_EXT_NCPSG, false, true, true>;
template [[host_name("kernel_flash_attn_ext_tqk3_sjj_d256_tqv4_0_d256_dk256_dv256")]]    kernel flash_attn_ext_t kernel_flash_attn_ext<FA_TYPES, block_tqk_3hi_2lo_d256, 16, dequantize_3hi_2lo_had_d256, block_tqv_had_mse4_d256, 16, dequantize_had_mse4_d256, 256, 256, OP_FLASH_ATTN_EXT_NQPSG, OP_FLASH_ATTN_EXT_NCPSG, false, true, true, true>;
...
kernel void kernel_flash_attn_ext_vec(
    ...
    threadgroup half tq_vec_tmp[DK]; // scratch for structured rotation permutation step
...
#define NS20 (FC_flash_attn_ext_vec_ns20)
  ...
  //threadgroup q_t   * sq  = (threadgroup q_t   *) (shmem_f16 +                      0*PK); // holds the query data
    ...
    if (TQ_SPLIT) {
        // Permuted Q load: rearrange channels to [outlier, regular] order
        device const uint8_t * perm = (device const uint8_t *)tq_chmap + ikv2 * DK;
        device const float * q_head = (device const float *) q;
        threadgroup half * sq_h = (threadgroup half *)sq4;
        if (iq1 < args.ne01) {
            for (short i = tiisg; i < DK; i += NW) {
                sq_h[i] = half(q_head[perm[i]]);
            }
            for (short i = DK + tiisg; i < PK; i += NW) {
                sq_h[i] = 0;
            }
        }
        // Apply rotation: FWHT on hi, rotation on lo
        // TQ_SPLIT_STRUCT: full structured rotation for _sjj
        // !TQ_SPLIT_STRUCT: bare 3×FWHT for _sj
        threadgroup_barrier(mem_flags::mem_threadgroup);
        if (sgitg == 0 && iq1 < args.ne01) {
            ...
            // Hi: plain FWHT
            tq_fwht_shared<DK/4>(sq_h, tiisg);

            if (TQ_SPLIT_STRUCT) {
                // Lo: steps 1+2 fused (sign + O_3 mix)
                for (short col = tiisg; col < blk_dim; col += NW) {
                    float a = float(sq_h[lo_off + col])             * tq_struct_sign(col, lo_n);
                    float b = float(sq_h[lo_off + blk_dim + col])   * tq_struct_sign(blk_dim + col, lo_n);
                    float c = float(sq_h[lo_off + 2*blk_dim + col]) * tq_struct_sign(2*blk_dim + col, lo_n);
                    tq_vec_tmp[col]             = half(tq_O3[0]*a + tq_O3[1]*b + tq_O3[2]*c);
                    tq_vec_tmp[blk_dim + col]   = half(tq_O3[3]*a + tq_O3[4]*b + tq_O3[5]*c);
                    tq_vec_tmp[2*blk_dim + col] = half(tq_O3[6]*a + tq_O3[7]*b + tq_O3[8]*c);
                }
                simdgroup_barrier(mem_flags::mem_threadgroup);

                // Lo: step 3 — permutation
                for (short i = tiisg; i < lo_n; i += NW) {
                    sq_h[lo_off + (35*i + 17) % lo_n] = tq_vec_tmp[i];
                }
                simdgroup_barrier(mem_flags::mem_threadgroup);
            }
            ...
            // Lo: 3 × FWHT (step 4 of structured, or sole step for bare)
            for (short blki = 0; blki < lo_n; blki += blk_dim) {
                tq_fwht_shared<DK/4>(sq_h + lo_off + blki, tiisg);
            }
        }
    } else {
...
// TurboQuant K (rotated space, Q pre-rotated with fused FWHT) + f16 V
template [[host_name("kernel_flash_attn_ext_vec_tqk4_0_f16_dk128_dv128")]]  kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_tqk_had_mse4,  32, dequantize_had_mse4_t4,  half4, 1, dequantize_f16_t4, 128, 128, 1, OP_FLASH_ATTN_EXT_VEC_NQPSG, OP_FLASH_ATTN_EXT_VEC_NCPSG, true>;
template [[host_name("kernel_flash_attn_ext_vec_tqk5_0j_f16_dk128_dv128")]] kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_tqk_had_prod5, 32, dequantize_had_prod5_t4, half4, 1, dequantize_f16_t4, 128, 128, 1, OP_FLASH_ATTN_EXT_VEC_NQPSG, OP_FLASH_ATTN_EXT_VEC_NCPSG, true>;
template [[host_name("kernel_flash_attn_ext_vec_tqk4_1j_f16_dk128_dv128")]] kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_tqk_had_prod4, 32, dequantize_had_prod4_t4, half4, 1, dequantize_f16_t4, 128, 128, 1, OP_FLASH_ATTN_EXT_VEC_NQPSG, OP_FLASH_ATTN_EXT_VEC_NCPSG, true>;
template [[host_name("kernel_flash_attn_ext_vec_tqk3_sj_f16_dk128_dv128")]] kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_tqk_5hi_3lo, 32, dequantize_5hi_3lo_had_t4, half4, 1, dequantize_f16_t4, 128, 128, 1, OP_FLASH_ATTN_EXT_VEC_NQPSG, OP_FLASH_ATTN_EXT_VEC_NCPSG, false, true>;
// TQ K + TQV V (V stored rotated, inverse FWHT on output)
template [[host_name("kernel_flash_attn_ext_vec_tqk4_0_tqv4_0_dk128_dv128")]]  kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_tqk_had_mse4,  32, dequantize_had_mse4_t4,  block_tqv_had_mse4, 32, dequantize_had_mse4_t4, 128, 128, 1, OP_FLASH_ATTN_EXT_VEC_NQPSG, OP_FLASH_ATTN_EXT_VEC_NCPSG, true,  false, false, true>;
template [[host_name("kernel_flash_attn_ext_vec_tqk5_0j_tqv4_0_dk128_dv128")]] kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_tqk_had_prod5, 32, dequantize_had_prod5_t4, block_tqv_had_mse4, 32, dequantize_had_mse4_t4, 128, 128, 1, OP_FLASH_ATTN_EXT_VEC_NQPSG, OP_FLASH_ATTN_EXT_VEC_NCPSG, true,  false, false, true>;
template [[host_name("kernel_flash_attn_ext_vec_tqk4_1j_tqv4_0_dk128_dv128")]] kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_tqk_had_prod4, 32, dequantize_had_prod4_t4, block_tqv_had_mse4, 32, dequantize_had_mse4_t4, 128, 128, 1, OP_FLASH_ATTN_EXT_VEC_NQPSG, OP_FLASH_ATTN_EXT_VEC_NCPSG, true,  false, false, true>;
template [[host_name("kernel_flash_attn_ext_vec_tqk3_sj_tqv4_0_dk128_dv128")]] kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_tqk_5hi_3lo, 32, dequantize_5hi_3lo_had_t4, block_tqv_had_mse4, 32, dequantize_had_mse4_t4, 128, 128, 1, OP_FLASH_ATTN_EXT_VEC_NQPSG, OP_FLASH_ATTN_EXT_VEC_NCPSG, false, true, false, true>;
// f16 K + TQV V (V stored rotated, inverse FWHT on output)
template [[host_name("kernel_flash_attn_ext_vec_f16_tqv4_0_dk128_dv128")]] kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, half4, 1, dequantize_f16_t4, block_tqv_had_mse4, 32, dequantize_had_mse4_t4, 128, 128, 1, OP_FLASH_ATTN_EXT_VEC_NQPSG, OP_FLASH_ATTN_EXT_VEC_NCPSG, false, false, false, true>;
// TQ K + q4_0 V
template [[host_name("kernel_flash_attn_ext_vec_tqk4_0_q4_0_dk128_dv128")]]  kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_tqk_had_mse4,  32, dequantize_had_mse4_t4,  block_q4_0, 8, dequantize_q4_0_t4, 128, 128, 1, OP_FLASH_ATTN_EXT_VEC_NQPSG, OP_FLASH_ATTN_EXT_VEC_NCPSG, true>;
template [[host_name("kernel_flash_attn_ext_vec_tqk5_0j_q4_0_dk128_dv128")]] kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_tqk_had_prod5, 32, dequantize_had_prod5_t4, block_q4_0, 8, dequantize_q4_0_t4, 128, 128, 1, OP_FLASH_ATTN_EXT_VEC_NQPSG, OP_FLASH_ATTN_EXT_VEC_NCPSG, true>;
template [[host_name("kernel_flash_attn_ext_vec_tqk4_1j_q4_0_dk128_dv128")]] kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_tqk_had_prod4, 32, dequantize_had_prod4_t4, block_q4_0, 8, dequantize_q4_0_t4, 128, 128, 1, OP_FLASH_ATTN_EXT_VEC_NQPSG, OP_FLASH_ATTN_EXT_VEC_NCPSG, true>;
template [[host_name("kernel_flash_attn_ext_vec_tqk3_sj_q4_0_dk128_dv128")]] kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_tqk_5hi_3lo, 32, dequantize_5hi_3lo_had_t4, block_q4_0, 8, dequantize_q4_0_t4, 128, 128, 1, OP_FLASH_ATTN_EXT_VEC_NQPSG, OP_FLASH_ATTN_EXT_VEC_NCPSG, false, true>;
// TQK 6hi_3lo (tqk4_sj) vec FA
...
// TQK 5r3_sj (tqk5r3_sj) vec FA — 5-bit+3-bit residual hi, 3-bit lo, no QJL
template [[host_name("kernel_flash_attn_ext_vec_tqk5r3_sj_f16_dk128_dv128")]]      kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_tqk_5r3_sj, 32, dequantize_5r3_sj_t4, half4, 1, dequantize_f16_t4, 128, 128, 1, OP_FLASH_ATTN_EXT_VEC_NQPSG, OP_FLASH_ATTN_EXT_VEC_NCPSG, false, true>;
template [[host_name("kernel_flash_attn_ext_vec_tqk5r3_sj_tqv4_0_dk128_dv128")]]   kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_tqk_5r3_sj, 32, dequantize_5r3_sj_t4, block_tqv_had_mse4, 32, dequantize_had_mse4_t4, 128, 128, 1, OP_FLASH_ATTN_EXT_VEC_NQPSG, OP_FLASH_ATTN_EXT_VEC_NCPSG, false, true, false, true>;
template [[host_name("kernel_flash_attn_ext_vec_tqk5r3_sj_q4_0_dk128_dv128")]]     kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_tqk_5r3_sj, 32, dequantize_5r3_sj_t4, block_q4_0, 8, dequantize_q4_0_t4, 128, 128, 1, OP_FLASH_ATTN_EXT_VEC_NQPSG, OP_FLASH_ATTN_EXT_VEC_NCPSG, false, true>;
// TQK 6hi_3lo_jj (tqk4_sjj) vec FA — shares dequant with tqk4_sj
template [[host_name("kernel_flash_attn_ext_vec_tqk4_sjj_f16_dk128_dv128")]]       kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_tqk_6hi_3lo_jj, 32, dequantize_6hi_3lo_jj_had_t4, half4, 1, dequantize_f16_t4, 128, 128, 1, OP_FLASH_ATTN_EXT_VEC_NQPSG, OP_FLASH_ATTN_EXT_VEC_NCPSG, false, true, true>;
template [[host_name("kernel_flash_attn_ext_vec_tqk4_sjj_tqv4_0_dk128_dv128")]]    kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_tqk_6hi_3lo_jj, 32, dequantize_6hi_3lo_jj_had_t4, block_tqv_had_mse4, 32, dequantize_had_mse4_t4, 128, 128, 1, OP_FLASH_ATTN_EXT_VEC_NQPSG, OP_FLASH_ATTN_EXT_VEC_NCPSG, false, true, true, true>;
template [[host_name("kernel_flash_attn_ext_vec_tqk4_sjj_q4_0_dk128_dv128")]]      kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_tqk_6hi_3lo_jj, 32, dequantize_6hi_3lo_jj_had_t4, block_q4_0, 8, dequantize_q4_0_t4, 128, 128, 1, OP_FLASH_ATTN_EXT_VEC_NQPSG, OP_FLASH_ATTN_EXT_VEC_NCPSG, false, true, true>;
// TQK 2hi_1lo (tqk2_sjj) vec FA
template [[host_name("kernel_flash_attn_ext_vec_tqk2_sjj_f16_dk128_dv128")]]        kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_tqk_2hi_1lo, 32, dequantize_2hi_1lo_had_t4, half4, 1, dequantize_f16_t4, 128, 128, 1, OP_FLASH_ATTN_EXT_VEC_NQPSG, OP_FLASH_ATTN_EXT_VEC_NCPSG, false, true, true>;
template [[host_name("kernel_flash_attn_ext_vec_tqk2_sjj_tqv4_0_dk128_dv128")]]     kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_tqk_2hi_1lo, 32, dequantize_2hi_1lo_had_t4, block_tqv_had_mse4, 32, dequantize_had_mse4_t4, 128, 128, 1, OP_FLASH_ATTN_EXT_VEC_NQPSG, OP_FLASH_ATTN_EXT_VEC_NCPSG, false, true, true, true>;
template [[host_name("kernel_flash_attn_ext_vec_tqk2_sjj_q4_0_dk128_dv128")]]       kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_tqk_2hi_1lo, 32, dequantize_2hi_1lo_had_t4, block_q4_0, 8, dequantize_q4_0_t4, 128, 128, 1, OP_FLASH_ATTN_EXT_VEC_NQPSG, OP_FLASH_ATTN_EXT_VEC_NCPSG, false, true, true>;
// TQK 3hi_2lo (tqk3_sjj) vec FA
template [[host_name("kernel_flash_attn_ext_vec_tqk3_sjj_f16_dk128_dv128")]]       kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_tqk_3hi_2lo, 32, dequantize_3hi_2lo_had_t4, half4, 1, dequantize_f16_t4, 128, 128, 1, OP_FLASH_ATTN_EXT_VEC_NQPSG, OP_FLASH_ATTN_EXT_VEC_NCPSG, false, true, true>;
template [[host_name("kernel_flash_attn_ext_vec_tqk3_sjj_tqv4_0_dk128_dv128")]]    kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_tqk_3hi_2lo, 32, dequantize_3hi_2lo_had_t4, block_tqv_had_mse4, 32, dequantize_had_mse4_t4, 128, 128, 1, OP_FLASH_ATTN_EXT_VEC_NQPSG, OP_FLASH_ATTN_EXT_VEC_NCPSG, false, true, true, true>;
template [[host_name("kernel_flash_attn_ext_vec_tqk3_sjj_q4_0_dk128_dv128")]]      kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_tqk_3hi_2lo, 32, dequantize_3hi_2lo_had_t4, block_q4_0, 8, dequantize_q4_0_t4, 128, 128, 1, OP_FLASH_ATTN_EXT_VEC_NQPSG, OP_FLASH_ATTN_EXT_VEC_NCPSG, false, true, true>;
...
// TurboQuant d=256 K + f16 V
template [[host_name("kernel_flash_attn_ext_vec_tqk4_0_d256_f16_dk256_dv256")]]  kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_tqk_had_mse4_d256,  64, dequantize_had_mse4_d256_t4,  half4, 1, dequantize_f16_t4, 256, 256, 1, OP_FLASH_ATTN_EXT_VEC_NQPSG, OP_FLASH_ATTN_EXT_VEC_NCPSG, true>;
template [[host_name("kernel_flash_attn_ext_vec_tqk5_0j_d256_f16_dk256_dv256")]] kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_tqk_had_prod5_d256, 64, dequantize_had_prod5_d256_t4, half4, 1, dequantize_f16_t4, 256, 256, 1, OP_FLASH_ATTN_EXT_VEC_NQPSG, OP_FLASH_ATTN_EXT_VEC_NCPSG, true>;
template [[host_name("kernel_flash_attn_ext_vec_tqk4_1j_d256_f16_dk256_dv256")]] kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_tqk_had_prod4_d256, 64, dequantize_had_prod4_d256_t4, half4, 1, dequantize_f16_t4, 256, 256, 1, OP_FLASH_ATTN_EXT_VEC_NQPSG, OP_FLASH_ATTN_EXT_VEC_NCPSG, true>;
// 5hi_3lo_had d=256 K + f16 V
template [[host_name("kernel_flash_attn_ext_vec_tqk3_sj_d256_f16_dk256_dv256")]] kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_tqk_5hi_3lo_d256, 64, dequantize_5hi_3lo_had_d256_t4, half4, 1, dequantize_f16_t4, 256, 256, 1, OP_FLASH_ATTN_EXT_VEC_NQPSG, OP_FLASH_ATTN_EXT_VEC_NCPSG, false, true>;
// f16 K + TQV d=256 V
template [[host_name("kernel_flash_attn_ext_vec_f16_tqv4_0_d256_dk256_dv256")]] kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, half4, 1, dequantize_f16_t4, block_tqv_had_mse4_d256, 64, dequantize_had_mse4_d256_t4, 256, 256, 1, OP_FLASH_ATTN_EXT_VEC_NQPSG, OP_FLASH_ATTN_EXT_VEC_NCPSG, false, false, false, true>;
// TQ d=256 K + TQV d=256 V
template [[host_name("kernel_flash_attn_ext_vec_tqk4_0_d256_tqv4_0_d256_dk256_dv256")]]  kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_tqk_had_mse4_d256,  64, dequantize_had_mse4_d256_t4,  block_tqv_had_mse4_d256, 64, dequantize_had_mse4_d256_t4, 256, 256, 1, OP_FLASH_ATTN_EXT_VEC_NQPSG, OP_FLASH_ATTN_EXT_VEC_NCPSG, true,  false, false, true>;
template [[host_name("kernel_flash_attn_ext_vec_tqk5_0j_d256_tqv4_0_d256_dk256_dv256")]] kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_tqk_had_prod5_d256, 64, dequantize_had_prod5_d256_t4, block_tqv_had_mse4_d256, 64, dequantize_had_mse4_d256_t4, 256, 256, 1, OP_FLASH_ATTN_EXT_VEC_NQPSG, OP_FLASH_ATTN_EXT_VEC_NCPSG, true,  false, false, true>;
template [[host_name("kernel_flash_attn_ext_vec_tqk4_1j_d256_tqv4_0_d256_dk256_dv256")]] kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_tqk_had_prod4_d256, 64, dequantize_had_prod4_d256_t4, block_tqv_had_mse4_d256, 64, dequantize_had_mse4_d256_t4, 256, 256, 1, OP_FLASH_ATTN_EXT_VEC_NQPSG, OP_FLASH_ATTN_EXT_VEC_NCPSG, true,  false, false, true>;
template [[host_name("kernel_flash_attn_ext_vec_tqk3_sj_d256_tqv4_0_d256_dk256_dv256")]] kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_tqk_5hi_3lo_d256, 64, dequantize_5hi_3lo_had_d256_t4, block_tqv_had_mse4_d256, 64, dequantize_had_mse4_d256_t4, 256, 256, 1, OP_FLASH_ATTN_EXT_VEC_NQPSG, OP_FLASH_ATTN_EXT_VEC_NCPSG, false, true, false, true>;
// 6hi_3lo_had d=256 K + f16/tqv V (vec)
template [[host_name("kernel_flash_attn_ext_vec_tqk4_sj_d256_f16_dk256_dv256")]]             kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_tqk_6hi_3lo_d256, 64, dequantize_6hi_3lo_had_d256_t4, half4, 1, dequantize_f16_t4, 256, 256, 1, OP_FLASH_ATTN_EXT_VEC_NQPSG, OP_FLASH_ATTN_EXT_VEC_NCPSG, false, true>;
template [[host_name("kernel_flash_attn_ext_vec_tqk4_sj_d256_tqv4_0_d256_dk256_dv256")]]     kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_tqk_6hi_3lo_d256, 64, dequantize_6hi_3lo_had_d256_t4, block_tqv_had_mse4_d256, 64, dequantize_had_mse4_d256_t4, 256, 256, 1, OP_FLASH_ATTN_EXT_VEC_NQPSG, OP_FLASH_ATTN_EXT_VEC_NCPSG, false, true, false, true>;
// 6hi_3lo_had_jj d=256 (tqk4_sjj) vec FA — shares dequant with tqk4_sj
template [[host_name("kernel_flash_attn_ext_vec_tqk4_sjj_d256_f16_dk256_dv256")]]            kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_tqk_6hi_3lo_jj_d256, 64, dequantize_6hi_3lo_jj_had_d256_t4, half4, 1, dequantize_f16_t4, 256, 256, 1, OP_FLASH_ATTN_EXT_VEC_NQPSG, OP_FLASH_ATTN_EXT_VEC_NCPSG, false, true, true>;
template [[host_name("kernel_flash_attn_ext_vec_tqk4_sjj_d256_tqv4_0_d256_dk256_dv256")]]    kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_tqk_6hi_3lo_jj_d256, 64, dequantize_6hi_3lo_jj_had_d256_t4, block_tqv_had_mse4_d256, 64, dequantize_had_mse4_d256_t4, 256, 256, 1, OP_FLASH_ATTN_EXT_VEC_NQPSG, OP_FLASH_ATTN_EXT_VEC_NCPSG, false, true, true, true>;
// 2hi_1lo_had d=256 K + f16/tqv V (vec)
template [[host_name("kernel_flash_attn_ext_vec_tqk2_sjj_d256_f16_dk256_dv256")]]             kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_tqk_2hi_1lo_d256, 64, dequantize_2hi_1lo_had_d256_t4, half4, 1, dequantize_f16_t4, 256, 256, 1, OP_FLASH_ATTN_EXT_VEC_NQPSG, OP_FLASH_ATTN_EXT_VEC_NCPSG, false, true, true>;
template [[host_name("kernel_flash_attn_ext_vec_tqk2_sjj_d256_tqv4_0_d256_dk256_dv256")]]     kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_tqk_2hi_1lo_d256, 64, dequantize_2hi_1lo_had_d256_t4, block_tqv_had_mse4_d256, 64, dequantize_had_mse4_d256_t4, 256, 256, 1, OP_FLASH_ATTN_EXT_VEC_NQPSG, OP_FLASH_ATTN_EXT_VEC_NCPSG, false, true, true, true>;
// 3hi_2lo_had d=256 K + f16/tqv V (vec)
template [[host_name("kernel_flash_attn_ext_vec_tqk3_sjj_d256_f16_dk256_dv256")]]            kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_tqk_3hi_2lo_d256, 64, dequantize_3hi_2lo_had_d256_t4, half4, 1, dequantize_f16_t4, 256, 256, 1, OP_FLASH_ATTN_EXT_VEC_NQPSG, OP_FLASH_ATTN_EXT_VEC_NCPSG, false, true, true>;
template [[host_name("kernel_flash_attn_ext_vec_tqk3_sjj_d256_tqv4_0_d256_dk256_dv256")]]    kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_tqk_3hi_2lo_d256, 64, dequantize_3hi_2lo_had_d256_t4, block_tqv_had_mse4_d256, 64, dequantize_had_mse4_d256_t4, 256, 256, 1, OP_FLASH_ATTN_EXT_VEC_NQPSG, OP_FLASH_ATTN_EXT_VEC_NCPSG, false, true, true, true>;
...
kernel void kernel_get_rows_5hi_3lo_had(
    ...
    // Dequant lo: centroid lookup → scale → bare 3×FWHT (no QJL on lo for _sj)
    thread float lo[96];
    for (int j = 0; j < 96; j++) lo[j] = tq_c8_d96[tq_up3(blk->qs_lo, j)] * norm_lo;
...
// 6hi_3lo_had (tqk4_sj): get_rows (dequant with calibrated channel map)
[[host_name("kernel_get_rows_6hi_3lo_had")]]
kernel void kernel_get_rows_6hi_3lo_had(
        constant ggml_metal_kargs_get_rows & args,
        device const void    * src0,
        device const void    * src1,
        device       float   * dst,
    ...
    const int iblk = tiitg;
    if (iblk >= args.ne00 / 128) return;
    device const block_tqk_6hi_3lo * blk = (device const block_tqk_6hi_3lo *)
        ((const device char *)src0 + i03*args.nb03 + i02*args.nb02 + r*args.nb01) + iblk;
    device const int32_t * perm = tq_chmap + (tq_layer_idx * tq_n_kv_heads + iblk) * 128;
    float norm_hi = float(blk->norm_hi), norm_lo = float(blk->norm_lo), rnorm_hi = float(blk->rnorm_hi);
    thread float hi[32];
    ...
    float qjl_s = 1.2533141f / 32.0f * rnorm_hi;
    for (int j = 0; j < 32; j++) hi[j] = norm_hi * hi[j] + qjl_s * corr[j];
    // Lo: MSE only, no QJL for _sj — bare 3×FWHT
    thread float lo[96];
    for (int j = 0; j < 96; j++) lo[j] = tq_c8_d96[tq_up3(blk->qs_lo, j)] * norm_lo;
    tq_fwht<32>(lo); tq_fwht<32>(lo + 32); tq_fwht<32>(lo + 64);
    device float * out = dst + (i10*args.ne00 + iblk*128) + i02*args.nb2/4 + i03*args.nb3/4;
...
// 5r3_sj (tqk5r3_sj): get_rows (5-bit+3-bit residual hi, 3-bit lo, no QJL)
[[host_name("kernel_get_rows_5r3_sj")]]
kernel void kernel_get_rows_5r3_sj(
        constant ggml_metal_kargs_get_rows & args,
        device const void    * src0,
        device const void    * src1,
        device       float   * dst,
    ...
    const int iblk = tiitg;
    if (iblk >= args.ne00 / 128) return;
    device const block_tqk_5r3_sj * blk = (device const block_tqk_5r3_sj *)
        ((const device char *)src0 + i03*args.nb03 + i02*args.nb02 + r*args.nb01) + iblk;
    device const int32_t * perm = tq_chmap + (tq_layer_idx * tq_n_kv_heads + iblk) * 128;
    float norm_hi = float(blk->norm_hi), norm_lo = float(blk->norm_lo), rnorm2 = float(blk->rnorm2_hi);
    // Hi: two-pass centroid dequant in rotated space, then inverse FWHT-32
    thread float hi[32];
    for (int j = 0; j < 32; j++) {
        hi[j] = norm_hi * tq_c32_d32[tq_up5(blk->qs_hi, j)] + rnorm2 * tq_c8_d32[tq_up3(blk->qs_hi2, j)];
    }
    tq_fwht<32>(hi);
    // Lo: 3-bit MSE, bare 3×FWHT inverse
    thread float lo[96];
    for (int j = 0; j < 96; j++) lo[j] = tq_c8_d96[tq_up3(blk->qs_lo, j)] * norm_lo;
    tq_fwht<32>(lo); tq_fwht<32>(lo + 32); tq_fwht<32>(lo + 64);
    device float * out = dst + (i10*args.ne00 + iblk*128) + i02*args.nb2/4 + i03*args.nb3/4;
...
// 6hi_3lo_had_jj (tqk4_sjj): get_rows (dequant with calibrated channel map, QJL on both hi and lo)
[[host_name("kernel_get_rows_6hi_3lo_had_jj")]]
kernel void kernel_get_rows_6hi_3lo_had_jj(
        constant ggml_metal_kargs_get_rows & args,
        device const void    * src0,
        device const void    * src1,
        device       float   * dst,
    ...
    const int iblk = tiitg;
    if (iblk >= args.ne00 / 128) return;
    device const block_tqk_6hi_3lo_jj * blk = (device const block_tqk_6hi_3lo_jj *)
        ((const device char *)src0 + i03*args.nb03 + i02*args.nb02 + r*args.nb01) + iblk;
    device const int32_t * perm = tq_chmap + (tq_layer_idx * tq_n_kv_heads + iblk) * 128;
...
// 6hi_3lo_had: set_rows (quantize with calibrated channel map, 5-bit MSE on hi)
[[host_name("kernel_set_rows_6hi_3lo_had_i32")]]
kernel void kernel_set_rows_6hi_3lo_had(
        constant ggml_metal_kargs_set_rows & args,
        device const void  * src0,
        device const void  * src1,
        device       void  * dst,
    ...
    tq_fwht<32>(lo_rot); tq_fwht<32>(lo_rot + 32); tq_fwht<32>(lo_rot + 64);
    ...
    device block_tqk_6hi_3lo * blk = (device block_tqk_6hi_3lo *)
        ((device char *)dst + i1*args.nb1 + i02*args.nb2 + i03*args.nb3) + iblk;
    blk->norm_hi = half(norm_hi); blk->norm_lo = half(norm_lo); blk->rnorm_hi = half(0.0f);
    if (norm_hi == 0 && norm_lo == 0) {
        for (int j = 0; j < 20; j++) blk->qs_hi[j] = 0;
    ...
    for (int j = 0; j < 20; j++) blk->qs_hi[j] = 0;
    for (int j = 0; j < 32; j++) {
        int idx = tq_nearest(hi_rot[j]*inv_hi, tq_c32_d32, 32);
        int bp = j*5, bi = bp/8, sh = bp%8;
        blk->qs_hi[bi] |= (uint8_t)((idx & 0x1F) << sh);
    ...
    float rnorm_sq=0; for (int j=0;j<32;j++) rnorm_sq+=r_hi[j]*r_hi[j];
    tq_fwht<32>(r_hi);
    for (int j=0;j<4;j++) blk->signs_hi[j]=0;
...
// 5r3_sj: set_rows (tqk5r3_sj — 5-bit MSE + 3-bit residual on hi, 3-bit MSE on lo, no QJL)
[[host_name("kernel_set_rows_5r3_sj_i32")]]
kernel void kernel_set_rows_5r3_sj(
        constant ggml_metal_kargs_set_rows & args,
        device const void  * src0,
        device const void  * src1,
        device       void  * dst,
    ...
    for (int j = 0; j < 96; j++) sum_lo += lo_rot[j]*lo_rot[j];
    float norm_hi = sqrt(sum_hi), norm_lo = sqrt(sum_lo);
    device block_tqk_5r3_sj * blk = (device block_tqk_5r3_sj *)
        ((device char *)dst + i1*args.nb1 + i02*args.nb2 + i03*args.nb3) + iblk;
    blk->norm_hi = half(norm_hi); blk->norm_lo = half(norm_lo); blk->rnorm2_hi = half(0.0f);
    if (norm_hi == 0 && norm_lo == 0) {
        for (int j = 0; j < 20; j++) blk->qs_hi[j] = 0;
        for (int j = 0; j < 12; j++) blk->qs_hi2[j] = 0;
        for (int j = 0; j < 36; j++) blk->qs_lo[j] = 0;
        return;
    }
    float inv_hi = (norm_hi > 1e-12f) ? 1.0f/norm_hi : 0, inv_lo = (norm_lo > 1e-12f) ? 1.0f/norm_lo : 0;
    ...
    float rnorm_sq = 0;
    for (int j = 0; j < 32; j++) {
        int bp=j*5,bi=bp/8,sh=bp%8; int v=blk->qs_hi[bi]>>sh; if(sh>3) v|=blk->qs_hi[bi+1]<<(8-sh);
        r1[j] = hi_rot[j] - norm_hi * tq_c32_d32[v & 0x1F];
        rnorm_sq += r1[j]*r1[j];
    }
    float rnorm2 = sqrt(rnorm_sq);
    float inv_rn = (rnorm2 > 1e-12f) ? 1.0f/rnorm2 : 0;
    // Second pass: 3-bit MSE on residual
    for (int j = 0; j < 12; j++) blk->qs_hi2[j] = 0;
    for (int j = 0; j < 32; j++) tq_pk3((device uint8_t *)blk->qs_hi2, j, tq_nearest(r1[j]*inv_rn, tq_c8_d32, 8));
    blk->rnorm2_hi = half(rnorm2);
    // Lo: 3-bit MSE
    for (int j = 0; j < 36; j++) blk->qs_lo[j] = 0;
    for (int j = 0; j < 96; j++) tq_pk3((device uint8_t *)blk->qs_lo, j, tq_nearest(lo_rot[j]*inv_lo, tq_c8_d96, 8));
}

// 6hi_3lo_had_jj: set_rows (tqk4_sjj — 5-bit MSE + QJL on hi, 3-bit MSE + QJL on lo)
[[host_name("kernel_set_rows_6hi_3lo_had_jj_i32")]]
kernel void kernel_set_rows_6hi_3lo_had_jj(
        constant ggml_metal_kargs_set_rows & args,
        device const void  * src0,
        device const void  * src1,
        device       void  * dst,
    ...
    tq_fwht<32>(hi_rot);
    for (int j = 0; j < 96; j++) lo_rot[j] = lo_raw[j];
    tq_structured_rotate_lo<96, 32>(lo_rot);
    float sum_hi = 0, sum_lo = 0;
    for (int j = 0; j < 32; j++) sum_hi += hi_rot[j]*hi_rot[j];
    for (int j = 0; j < 96; j++) sum_lo += lo_rot[j]*lo_rot[j];
    float norm_hi = sqrt(sum_hi), norm_lo = sqrt(sum_lo);
    device block_tqk_6hi_3lo_jj * blk = (device block_tqk_6hi_3lo_jj *)
        ((device char *)dst + i1*args.nb1 + i02*args.nb2 + i03*args.nb3) + iblk;
    blk->norm_hi = half(norm_hi); blk->norm_lo = half(norm_lo); blk->rnorm_hi = half(0.0f); blk->rnorm_lo = half(0.0f);
    if (norm_hi == 0 && norm_lo == 0) {
        for (int j = 0; j < 20; j++) blk->qs_hi[j] = 0;
        for (int j = 0; j < 36; j++) blk->qs_lo[j] = 0;
        for (int j = 0; j < 4; j++)  blk->signs_hi[j] = 0;
...
kernel void kernel_set_rows_2hi_1lo_had(
    ...
    for (int j = 0; j < 96; j++) lo_rot[j] = lo_raw[j]; tq_structured_rotate_lo<96, 32>(lo_rot);
    float sum_hi=0, sum_lo=0;
    for (int j=0;j<32;j++) sum_hi+=hi_rot[j]*hi_rot[j];
    for (int j=0;j<96;j++) sum_lo+=lo_rot[j]*lo_rot[j];
    float norm_hi=sqrt(sum_hi), norm_lo=sqrt(sum_lo);
...
// ---------------------------------------------------------------------------
// d=256 TurboQuant: get_rows and set_rows kernels
// ---------------------------------------------------------------------------

// had_mse4_d256: get_rows
[[host_name("kernel_get_rows_had_mse4_d256")]]
kernel void kernel_get_rows_had_mse4_d256(
        constant ggml_metal_kargs_get_rows & args,
        device const void  * src0,
        device const void  * src1,
        device       float * dst,
    ...
    const int iblk = tiitg;
    const int n_blocks = args.ne00 / 256;
    if (iblk >= n_blocks) return;

    device const block_tqk_had_mse4_d256 * blk = (device const block_tqk_had_mse4_d256 *)
        ((const device char *)src0 + i03*args.nb03 + i02*args.nb02 + r*args.nb01) + iblk;

    float norm = float(blk->norm);

    thread float rot[256];
    for (int j = 0; j < 256; j++) {
        rot[j] = tq_c16_d256[tq_up4(blk->qs, j)];
    }
    tq_fwht<256>(rot);
...
// had_mse4_d256: set_rows
[[host_name("kernel_set_rows_had_mse4_d256_i32")]]
kernel void kernel_set_rows_had_mse4_d256(
        constant ggml_metal_kargs_set_rows & args,
        device const void  * src0,
        device const void  * src1,
        device       void  * dst,
    ...
    device const float * src = (device const float *)((const device char *)src0 + i*args.nb01 + i02*args.nb02 + i03*args.nb03) + iblk*256;

    float sum_sq = 0.0f;
    thread float vals[256];
    for (int j = 0; j < 256; j++) {
        vals[j] = src[j];
        sum_sq += vals[j] * vals[j];
    }
    ...
    device block_tqk_had_mse4_d256 * blk = (device block_tqk_had_mse4_d256 *)
        ((device char *)dst + i1*args.nb1 + i02*args.nb2 + i03*args.nb3) + iblk;

    blk->norm = half(norm);

    if (norm == 0.0f) {
        for (int j = 0; j < 128; j++) blk->qs[j] = 0;
        return;
    }
    ...
    thread float rot[256];
    for (int j = 0; j < 256; j++) rot[j] = vals[j] * inv;
    tq_fwht<256>(rot);

    for (int j = 0; j < 128; j++) blk->qs[j] = 0;
    for (int j = 0; j < 256; j++) {
        tq_pk4((device uint8_t *)blk->qs, j, tq_nearest(rot[j], tq_c16_d256, 16));
    }
}

// had_prod5_d256: get_rows
[[host_name("kernel_get_rows_had_prod5_d256")]]
kernel void kernel_get_rows_had_prod5_d256(
        constant ggml_metal_kargs_get_rows & args,
        device const void  * src0,
        device const void  * src1,
        device       float * dst,
    ...
    device const block_tqk_had_prod5_d256 * blk = (device const block_tqk_had_prod5_d256 *)
        ((const device char *)src0 + i03*args.nb03 + i02*args.nb02 + r*args.nb01) + iblk;
    ...
    thread float corr[256];
    for (int j = 0; j < 256; j++) {
        corr[j] = ((blk->signs[j / 8] >> (j % 8)) & 1) ? 1.0f : -1.0f;
    }
    tq_fwht<256>(corr);
    float qjl_scale = 1.2533141f / 256.0f * rnorm;
...
// had_prod5_d256: set_rows
[[host_name("kernel_set_rows_had_prod5_d256_i32")]]
kernel void kernel_set_rows_had_prod5_d256(
        constant ggml_metal_kargs_set_rows & args,
        device const void  * src0,
        device const void  * src1,
        device       void  * dst,
    ...
    device block_tqk_had_prod5_d256 * blk = (device block_tqk_had_prod5_d256 *)
        ((device char *)dst + i1*args.nb1 + i02*args.nb2 + i03*args.nb3) + iblk;
    ...
    if (norm == 0.0f) {
        for (int j = 0; j < 128; j++) blk->qs[j] = 0;
        for (int j = 0; j < 32; j++) blk->signs[j] = 0;
        return;
    }
    ...
    thread float recon[256];
    for (int j = 0; j < 256; j++) recon[j] = tq_c16_d256[tq_up4(blk->qs, j)];
    tq_fwht<256>(recon);

    thread float resid[256];
    for (int j = 0; j < 256; j++) resid[j] = vals[j] - norm * recon[j];

    // QJL forward
    tq_fwht<256>(resid);
    float rnorm_sq = 0.0f;
    for (int j = 0; j < 256; j++) rnorm_sq += (vals[j] - norm * recon[j]) * (vals[j] - norm * recon[j]);
...
// had_prod4_d256: get_rows
[[host_name("kernel_get_rows_had_prod4_d256")]]
kernel void kernel_get_rows_had_prod4_d256(
        constant ggml_metal_kargs_get_rows & args,
        device const void  * src0,
        device const void  * src1,
        device       float * dst,
    ...
    device const block_tqk_had_prod4_d256 * blk = (device const block_tqk_had_prod4_d256 *)
        ((const device char *)src0 + i03*args.nb03 + i02*args.nb02 + r*args.nb01) + iblk;
    ...
    thread float rot[256];
    for (int j = 0; j < 256; j++) {
        rot[j] = tq_c8_d256[tq_up3(blk->qs, j)];
    }
    tq_fwht<256>(rot);
...
// had_prod4_d256: set_rows
[[host_name("kernel_set_rows_had_prod4_d256_i32")]]
kernel void kernel_set_rows_had_prod4_d256(
        constant ggml_metal_kargs_set_rows & args,
        device const void  * src0,
        device const void  * src1,
        device       void  * dst,
    ...
    device block_tqk_had_prod4_d256 * blk = (device block_tqk_had_prod4_d256 *)
        ((device char *)dst + i1*args.nb1 + i02*args.nb2 + i03*args.nb3) + iblk;
    ...
    for (int j = 0; j < 96; j++) blk->qs[j] = 0;
    for (int j = 0; j < 256; j++) {
        tq_pk3((device uint8_t *)blk->qs, j, tq_nearest(rot[j], tq_c8_d256, 8));
    }

    // Compute residual
    thread float recon[256];
    for (int j = 0; j < 256; j++) recon[j] = tq_c8_d256[tq_up3(blk->qs, j)];
    tq_fwht<256>(recon);
    ...
    // QJL forward
    float rnorm_sq = 0.0f;
    for (int j = 0; j < 256; j++) rnorm_sq += resid[j] * resid[j];
    tq_fwht<256>(resid);
...
[[host_name("kernel_set_rows_5hi_3lo_had_d256_i32")]]
kernel void kernel_set_rows_5hi_3lo_had_d256(
        constant ggml_metal_kargs_set_rows & args,
        device const void  * src0,
        device const void  * src1,
        device       void  * dst,
    ...
    // Get channel permutation for this head
    device const int32_t * perm = tq_chmap + (tq_layer_idx * tq_n_kv_heads + iblk) * 256;

    // Split using calibrated channel order
    thread float hi_raw[64], lo_raw[192];
    for (int j = 0; j < 64; j++)   hi_raw[j] = src[perm[j]];
    for (int j = 0; j < 192; j++)  lo_raw[j] = src[perm[64 + j]];

    // FWHT rotate
    thread float hi_rot[64], lo_rot[192];
    for (int j = 0; j < 64; j++) hi_rot[j] = hi_raw[j];
    tq_fwht<64>(hi_rot);
    for (int j = 0; j < 192; j++) lo_rot[j] = lo_raw[j];
    tq_fwht<64>(lo_rot); tq_fwht<64>(lo_rot + 64); tq_fwht<64>(lo_rot + 128);
    ...
    device block_tqk_5hi_3lo_d256 * blk = (device block_tqk_5hi_3lo_d256 *)
        ((device char *)dst + i1*args.nb1 + i02*args.nb2 + i03*args.nb3) + iblk;
    ...
    if (norm_hi == 0.0f && norm_lo == 0.0f) {
        for (int j = 0; j < 32; j++) blk->qs_hi[j] = 0;
        for (int j = 0; j < 72; j++) blk->qs_lo[j] = 0;
    ...
    for (int j = 0; j < 32; j++) blk->qs_hi[j] = 0;
    for (int j = 0; j < 64; j++) {
        tq_pk4((device uint8_t *)blk->qs_hi, j, tq_nearest(hi_rot[j] * inv_hi, tq_c16_d64, 16));
    }

    // 3-bit MSE for lo
    for (int j = 0; j < 72; j++) blk->qs_lo[j] = 0;
    for (int j = 0; j < 192; j++) {
        tq_pk3((device uint8_t *)blk->qs_lo, j, tq_nearest(lo_rot[j] * inv_lo, tq_c8_d192, 8));
    }

    // QJL on hi residual
    thread float yhi[64];
    for (int j = 0; j < 64; j++) yhi[j] = tq_c16_d64[tq_up4(blk->qs_hi, j)];
    thread float hi_rec[64];
    for (int j = 0; j < 64; j++) hi_rec[j] = yhi[j];
    tq_fwht<64>(hi_rec);
    thread float r_hi[64];
    for (int j = 0; j < 64; j++) r_hi[j] = hi_raw[j] - norm_hi * hi_rec[j];
    float rnorm_sq = 0.0f;
    for (int j = 0; j < 64; j++) rnorm_sq += r_hi[j] * r_hi[j];
    tq_fwht<64>(r_hi); // FWHT projection for signs
    for (int j = 0; j < 8; j++) blk->signs_hi[j] = 0;
    for (int j = 0; j < 64; j++) {
        if (r_hi[j] >= 0.0f) blk->signs_hi[j / 8] |= (uint8_t)(1 << (j % 8));
    }
...
// 6hi_3lo_had_d256: get_rows (5-bit hi + QJL, 3-bit lo, channel map)
[[host_name("kernel_get_rows_6hi_3lo_had_d256")]]
kernel void kernel_get_rows_6hi_3lo_had_d256(
        constant ggml_metal_kargs_get_rows & args,
        device const void    * src0,
        device const void    * src1,
        device       float   * dst,
    ...
    const int iblk = tiitg;
    if (iblk >= args.ne00 / 256) return;
    device const block_tqk_6hi_3lo_d256 * blk = (device const block_tqk_6hi_3lo_d256 *)
        ((const device char *)src0 + i03*args.nb03 + i02*args.nb02 + r*args.nb01) + iblk;
    device const int32_t * perm = tq_chmap + (tq_layer_idx * tq_n_kv_heads + iblk) * 256;
    float norm_hi = float(blk->norm_hi), norm_lo = float(blk->norm_lo), rnorm_hi = float(blk->rnorm_hi);
    thread float hi[64];
    ...
    // Lo: MSE only, no QJL for _sj d256 — bare 3×FWHT
    thread float lo[192];
    for (int j = 0; j < 192; j++) lo[j] = tq_c8_d192[tq_up3(blk->qs_lo, j)] * norm_lo;
    tq_fwht<64>(lo); tq_fwht<64>(lo + 64); tq_fwht<64>(lo + 128);
    device float * out = dst + (i10*args.ne00 + iblk*256) + i02*args.nb2/4 + i03*args.nb3/4;
...
kernel void kernel_set_rows_6hi_3lo_had_d256(
    ...
    for (int j = 0; j < 192; j++) lo_raw[j] = src[perm[64 + j]];
    thread float hi_rot[64], lo_rot[192];
    for (int j = 0; j < 64; j++) hi_rot[j] = hi_raw[j]; tq_fwht<64>(hi_rot);
    for (int j = 0; j < 192; j++) lo_rot[j] = lo_raw[j]; tq_fwht<64>(lo_rot); tq_fwht<64>(lo_rot + 64); tq_fwht<64>(lo_rot + 128);
    float sum_hi = 0, sum_lo = 0;
    for (int j = 0; j < 64; j++)  sum_hi += hi_rot[j]*hi_rot[j];
    for (int j = 0; j < 192; j++) sum_lo += lo_rot[j]*lo_rot[j];
    float norm_hi = sqrt(sum_hi), norm_lo = sqrt(sum_lo);
    device block_tqk_6hi_3lo_d256 * blk = (device block_tqk_6hi_3lo_d256 *)
        ((device char *)dst + i1*args.nb1 + i02*args.nb2 + i03*args.nb3) + iblk;
    blk->norm_hi = half(norm_hi); blk->norm_lo = half(norm_lo); blk->rnorm_hi = half(0.0f);
    if (norm_hi == 0 && norm_lo == 0) {
        for (int j = 0; j < 40; j++) blk->qs_hi[j] = 0;
        for (int j = 0; j < 72; j++) blk->qs_lo[j] = 0;
        for (int j = 0; j < 8; j++)  blk->signs_hi[j] = 0;
        return;
    }
    float inv_hi = (norm_hi > 1e-12f) ? 1.0f/norm_hi : 0, inv_lo = (norm_lo > 1e-12f) ? 1.0f/norm_lo : 0;
    // 5-bit MSE for hi (inline packing — no tq_pk5 helper)
    for (int j = 0; j < 40; j++) blk->qs_hi[j] = 0;
    for (int j = 0; j < 64; j++) {
        int idx = tq_nearest(hi_rot[j]*inv_hi, tq_c32_d64, 32);
        int bp = j*5, bi = bp/8, sh = bp%8;
        blk->qs_hi[bi] |= (uint8_t)((idx & 0x1F) << sh);
        if (sh > 3) blk->qs_hi[bi+1] |= (uint8_t)((idx & 0x1F) >> (8 - sh));
    }
    // 3-bit MSE for lo
    for (int j = 0; j < 72; j++) blk->qs_lo[j] = 0;
    for (int j = 0; j < 192; j++) tq_pk3((device uint8_t *)blk->qs_lo, j, tq_nearest(lo_rot[j]*inv_lo, tq_c8_d192, 8));
    // QJL on hi residual
    thread float yhi[64];
    for (int j = 0; j < 64; j++) { int bp=j*5,bi=bp/8,sh=bp%8; int v=blk->qs_hi[bi]>>sh; if(sh>3) v|=blk->qs_hi[bi+1]<<(8-sh); yhi[j]=tq_c32_d64[v&0x1F]; }
    thread float hi_rec[64]; for (int j=0;j<64;j++) hi_rec[j]=yhi[j]; tq_fwht<64>(hi_rec);
    thread float r_hi[64]; for (int j=0;j<64;j++) r_hi[j]=hi_raw[j]-norm_hi*hi_rec[j];
    float rnorm_sq=0; for (int j=0;j<64;j++) rnorm_sq+=r_hi[j]*r_hi[j];
    tq_fwht<64>(r_hi);
    for (int j=0;j<8;j++) blk->signs_hi[j]=0;
...
// 6hi_3lo_had_jj_d256: set_rows (tqk4_sjj d256 — 5-bit MSE + QJL on hi, 3-bit MSE + QJL on lo)
[[host_name("kernel_set_rows_6hi_3lo_had_jj_d256_i32")]]
kernel void kernel_set_rows_6hi_3lo_had_jj_d256(
        constant ggml_metal_kargs_set_rows & args,
        device const void  * src0,
        device const void  * src1,
        device       void  * dst,
    ...
    thread float hi_rot[64], lo_rot[192];
    for (int j = 0; j < 64; j++) hi_rot[j] = hi_raw[j]; tq_fwht<64>(hi_rot);
    for (int j = 0; j < 192; j++) lo_rot[j] = lo_raw[j]; tq_structured_rotate_lo<192, 64>(lo_rot);
    float sum_hi = 0, sum_lo = 0;
    for (int j = 0; j < 64; j++)  sum_hi += hi_rot[j]*hi_rot[j];
    for (int j = 0; j < 192; j++) sum_lo += lo_rot[j]*lo_rot[j];
    float norm_hi = sqrt(sum_hi), norm_lo = sqrt(sum_lo);
    device block_tqk_6hi_3lo_jj_d256 * blk = (device block_tqk_6hi_3lo_jj_d256 *)
        ((device char *)dst + i1*args.nb1 + i02*args.nb2 + i03*args.nb3) + iblk;
    blk->norm_hi = half(norm_hi); blk->norm_lo = half(norm_lo); blk->rnorm_hi = half(0.0f); blk->rnorm_lo = half(0.0f);
    if (norm_hi == 0 && norm_lo == 0) {
        for (int j = 0; j < 40; j++) blk->qs_hi[j] = 0;
...
// 2hi_1lo_had_d256: get_rows (2-bit hi + QJL, 1-bit lo + QJL, channel map)
[[host_name("kernel_get_rows_2hi_1lo_had_d256")]]
kernel void kernel_get_rows_2hi_1lo_had_d256(
        constant ggml_metal_kargs_get_rows & args,
        device const void    * src0,
        device const void    * src1,
        device       float   * dst,
    ...
    const int iblk = tiitg;
    if (iblk >= args.ne00 / 256) return;
    device const block_tqk_2hi_1lo_d256 * blk = (device const block_tqk_2hi_1lo_d256 *)
        ((const device char *)src0 + i03*args.nb03 + i02*args.nb02 + r*args.nb01) + iblk;
    device const int32_t * perm = tq_chmap + (tq_layer_idx * tq_n_kv_heads + iblk) * 256;
    float norm_hi = float(blk->norm_hi), norm_lo = float(blk->norm_lo);
    float rnorm_hi = float(blk->rnorm_hi), rnorm_lo = float(blk->rnorm_lo);
    thread float hi[64];
    for (int j = 0; j < 64; j++) hi[j] = tq_c4_d64[tq_up2(blk->qs_hi, j)];
    tq_fwht<64>(hi);
    thread float corr_hi[64];
    for (int j = 0; j < 64; j++) corr_hi[j] = ((blk->signs_hi[j/8] >> (j%8)) & 1) ? 1.0f : -1.0f;
    tq_fwht<64>(corr_hi);
...
// 2hi_1lo_had_d256: set_rows (2-bit hi + QJL, 1-bit lo + QJL, channel map)
[[host_name("kernel_set_rows_2hi_1lo_had_d256_i32")]]
kernel void kernel_set_rows_2hi_1lo_had_d256(
        constant ggml_metal_kargs_set_rows & args,
        device const void  * src0, device const void  * src1, device void * dst,
        device const int32_t * tq_chmap,
        constant int32_t & tq_layer_idx, constant int32_t & tq_n_kv_heads,
    ...
    for (int j=0;j<192;j++) sum_lo+=lo_rot[j]*lo_rot[j];
    float norm_hi=sqrt(sum_hi), norm_lo=sqrt(sum_lo);
    device block_tqk_2hi_1lo_d256 * blk = (device block_tqk_2hi_1lo_d256 *)((device char *)dst + i1*args.nb1 + i02*args.nb2 + i03*args.nb3) + iblk;
    blk->norm_hi=half(norm_hi); blk->norm_lo=half(norm_lo); blk->rnorm_hi=half(0); blk->rnorm_lo=half(0);
    if (norm_hi==0 && norm_lo==0) {
        for (int j=0;j<16;j++) blk->qs_hi[j]=0; for (int j=0;j<24;j++) blk->qs_lo[j]=0;
        for (int j=0;j<8;j++) blk->signs_hi[j]=0; for (int j=0;j<24;j++) blk->signs_lo[j]=0;
        return;
    }
    float inv_hi=(norm_hi>1e-12f)?1.0f/norm_hi:0, inv_lo=(norm_lo>1e-12f)?1.0f/norm_lo:0;
    for (int j=0;j<16;j++) blk->qs_hi[j]=0;
    for (int j=0;j<64;j++) tq_pk2((device uint8_t*)blk->qs_hi, j, tq_nearest(hi_rot[j]*inv_hi, tq_c4_d64, 4));
    for (int j=0;j<24;j++) blk->qs_lo[j]=0;
    for (int j=0;j<192;j++) tq_pk1((device uint8_t*)blk->qs_lo, j, tq_nearest(lo_rot[j]*inv_lo, tq_c2_d192, 2));
    // QJL on hi
    thread float yhi[64]; for (int j=0;j<64;j++) yhi[j]=tq_c4_d64[tq_up2(blk->qs_hi,j)];
    thread float hi_rec[64]; for (int j=0;j<64;j++) hi_rec[j]=yhi[j]; tq_fwht<64>(hi_rec);
    thread float r_hi[64]; for (int j=0;j<64;j++) r_hi[j]=hi_raw[j]-norm_hi*hi_rec[j];
    float rn_hi=0; for (int j=0;j<64;j++) rn_hi+=r_hi[j]*r_hi[j]; tq_fwht<64>(r_hi);
    for (int j=0;j<8;j++) blk->signs_hi[j]=0;
    for (int j=0;j<64;j++) { if (r_hi[j]>=0) blk->signs_hi[j/8]|=(uint8_t)(1<<(j%8)); }
    blk->rnorm_hi=half(sqrt(rn_hi));
    // QJL on lo (residual in rotated space — per-element sign, no FWHT)
...
// 3hi_2lo_had_d256: get_rows (3-bit hi + QJL, 2-bit lo + QJL, channel map)
[[host_name("kernel_get_rows_3hi_2lo_had_d256")]]
kernel void kernel_get_rows_3hi_2lo_had_d256(
        constant ggml_metal_kargs_get_rows & args,
        device const void    * src0,
        device const void    * src1,
        device       float   * dst,
    ...
    const int iblk = tiitg;
    if (iblk >= args.ne00 / 256) return;
    device const block_tqk_3hi_2lo_d256 * blk = (device const block_tqk_3hi_2lo_d256 *)
        ((const device char *)src0 + i03*args.nb03 + i02*args.nb02 + r*args.nb01) + iblk;
    device const int32_t * perm = tq_chmap + (tq_layer_idx * tq_n_kv_heads + iblk) * 256;
    float norm_hi = float(blk->norm_hi), norm_lo = float(blk->norm_lo);
    float rnorm_hi = float(blk->rnorm_hi), rnorm_lo = float(blk->rnorm_lo);
    thread float hi[64];
    for (int j = 0; j < 64; j++) hi[j] = tq_c8_d64[tq_up3(blk->qs_hi, j)];
    tq_fwht<64>(hi);
    thread float corr_hi[64];
    for (int j = 0; j < 64; j++) corr_hi[j] = ((blk->signs_hi[j/8] >> (j%8)) & 1) ? 1.0f : -1.0f;
    tq_fwht<64>(corr_hi);
...
// 3hi_2lo_had_d256: set_rows (3-bit hi + QJL, 2-bit lo + QJL, channel map)
[[host_name("kernel_set_rows_3hi_2lo_had_d256_i32")]]
kernel void kernel_set_rows_3hi_2lo_had_d256(
        constant ggml_metal_kargs_set_rows & args,
        device const void  * src0, device const void  * src1, device void * dst,
        device const int32_t * tq_chmap,
        constant int32_t & tq_layer_idx, constant int32_t & tq_n_kv_heads,
    ...
    for (int j=0;j<192;j++) sum_lo+=lo_rot[j]*lo_rot[j];
    float norm_hi=sqrt(sum_hi), norm_lo=sqrt(sum_lo);
    device block_tqk_3hi_2lo_d256 * blk = (device block_tqk_3hi_2lo_d256 *)((device char *)dst + i1*args.nb1 + i02*args.nb2 + i03*args.nb3) + iblk;
    blk->norm_hi=half(norm_hi); blk->norm_lo=half(norm_lo); blk->rnorm_hi=half(0); blk->rnorm_lo=half(0);
    if (norm_hi==0 && norm_lo==0) {
        for (int j=0;j<24;j++) blk->qs_hi[j]=0; for (int j=0;j<48;j++) blk->qs_lo[j]=0;
        for (int j=0;j<8;j++) blk->signs_hi[j]=0; for (int j=0;j<24;j++) blk->signs_lo[j]=0;
        return;
    }
    float inv_hi=(norm_hi>1e-12f)?1.0f/norm_hi:0, inv_lo=(norm_lo>1e-12f)?1.0f/norm_lo:0;
    for (int j=0;j<24;j++) blk->qs_hi[j]=0;
    for (int j=0;j<64;j++) tq_pk3((device uint8_t*)blk->qs_hi, j, tq_nearest(hi_rot[j]*inv_hi, tq_c8_d64, 8));
    for (int j=0;j<48;j++) blk->qs_lo[j]=0;
    for (int j=0;j<192;j++) tq_pk2((device uint8_t*)blk->qs_lo, j, tq_nearest(lo_rot[j]*inv_lo, tq_c4_d192, 4));
    // QJL on hi
    thread float yhi[64]; for (int j=0;j<64;j++) yhi[j]=tq_c8_d64[tq_up3(blk->qs_hi,j)];
    thread float hi_rec[64]; for (int j=0;j<64;j++) hi_rec[j]=yhi[j]; tq_fwht<64>(hi_rec);
    thread float r_hi[64]; for (int j=0;j<64;j++) r_hi[j]=hi_raw[j]-norm_hi*hi_rec[j];
    float rn_hi=0; for (int j=0;j<64;j++) rn_hi+=r_hi[j]*r_hi[j]; tq_fwht<64>(r_hi);
    for (int j=0;j<8;j++) blk->signs_hi[j]=0;

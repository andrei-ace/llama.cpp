...
// TQ3/TQ2: MSE-only dequant for V cache — outputs FWHT-space values (pointwise, fast)
// The FA kernel applies inverse FWHT to V shared memory after loading via TQ_FWHT_O
template <short BLK_DIM = 128, typename type4x4>
...
template <short BLK_DIM = 128, typename type4x4>
void dequantize_tq2(device const block_tq2 * xb_struct, short il, thread type4x4 & reg) {
    device const uint8_t * xb = (device const uint8_t *)xb_struct;
    const float norm = float(*(device const half *)(xb));
    device const uint8_t * qs = xb + 2;
    constant const float * c = (BLK_DIM >= 512) ? tq_c4_d512 : (BLK_DIM == 256) ? tq_c4_d256 : tq_c4_d128;
    float4x4 reg_f;
    for (int i = 0; i < 16; i++) reg_f[i/4][i%4] = norm * c[tq_up2(qs, il * 16 + i)];
    reg = (type4x4) reg_f;
}

template <short BLK_DIM = 128, typename type4>
void dequantize_tq3_t4(device const block_tq3 * xb_struct, short il, thread type4 & reg) {
    device const uint8_t * xb = (device const uint8_t *)xb_struct;
    const float norm = float(*(device const half *)(xb));
    device const uint8_t * qs = xb + 2;
    constant const float * c = (BLK_DIM >= 512) ? tq_c8_d512 : (BLK_DIM == 256) ? tq_c8_d256 : tq_c8_d128;
...
template <short BLK_DIM = 128, typename type4>
void dequantize_tq2_t4(device const block_tq2 * xb_struct, short il, thread type4 & reg) {
    device const uint8_t * xb = (device const uint8_t *)xb_struct;
    const float norm = float(*(device const half *)(xb));
    device const uint8_t * qs = xb + 2;
    constant const float * c = (BLK_DIM >= 512) ? tq_c4_d512 : (BLK_DIM == 256) ? tq_c4_d256 : tq_c4_d128;
...
template<
    ...
    bool TQ_FWHT = false,       // apply FWHT-DK to Q (tq3j/tq2j/tq3/tq2)
  ...
  //constexpr short PV16 = PV/16;
    ...
    if (TQ_FWHT_O) {
        ...
        for (short jj = 0; jj < NQ; ++jj) {
            ...
            if (DV4 % NW == 0) {
                FOR_UNROLL (short ii = 0; ii < DV4/NW; ++ii) {
                    dst4[ii*NW + tiisg] = (float4) so4[j*PV4 + ii*NW + tiisg]*scale;
                }
            } else {
                for (short i = tiisg; i < DV4; i += NW) {
...
#define FWD_ARGS args, q, k, v, mask, sinks, pad, blk, dst, shmem_f16, tgpig, tiisg, sgitg
    switch (FC_flash_attn_ext_nsg) {
        case 4: kernel_flash_attn_ext_impl<FWD_TMPL, 4, TQ_FWHT, TQ_FWHT_O>(FWD_ARGS); break;
        case 8: kernel_flash_attn_ext_impl<FWD_TMPL, 8, TQ_FWHT, TQ_FWHT_O>(FWD_ARGS); break;
    }
#undef FWD_TMPL
#undef FWD_ARGS
}
...
// TQ K-only (V=f16): type name matches ggml_type_name() for per-layer dispatch
// d=128
...
// d=256
template [[host_name("kernel_flash_attn_ext_tq3j_256_dk256_dv256")]] kernel flash_attn_ext_t kernel_flash_attn_ext<FA_TYPES, block_tq3j, 16, dequantize_tq3j<256>, half4x4, 1, dequantize_f16, 256, 256, OP_FLASH_ATTN_EXT_NQPSG, OP_FLASH_ATTN_EXT_NCPSG, true>;
template [[host_name("kernel_flash_attn_ext_tq2j_256_dk256_dv256")]] kernel flash_attn_ext_t kernel_flash_attn_ext<FA_TYPES, block_tq2j, 16, dequantize_tq2j<256>, half4x4, 1, dequantize_f16, 256, 256, OP_FLASH_ATTN_EXT_NQPSG, OP_FLASH_ATTN_EXT_NCPSG, true>;
// d=512
template [[host_name("kernel_flash_attn_ext_tq3j_512_dk512_dv512")]] kernel flash_attn_ext_t kernel_flash_attn_ext<FA_TYPES, block_tq3j, 32, dequantize_tq3j<512>, half4x4, 1, dequantize_f16, 512, 512, OP_FLASH_ATTN_EXT_NQPSG, OP_FLASH_ATTN_EXT_NCPSG, true>;
template [[host_name("kernel_flash_attn_ext_tq2j_512_dk512_dv512")]] kernel flash_attn_ext_t kernel_flash_attn_ext<FA_TYPES, block_tq2j, 32, dequantize_tq2j<512>, half4x4, 1, dequantize_f16, 512, 512, OP_FLASH_ATTN_EXT_NQPSG, OP_FLASH_ATTN_EXT_NCPSG, true>;

// Mixed K/V: K=tq3j/tq2j V=tq3/tq2 (V stored in FWHT space, output needs inverse FWHT)
// dk128
template [[host_name("kernel_flash_attn_ext_tq3j_tq3_dk128_dv128")]] kernel flash_attn_ext_t kernel_flash_attn_ext<FA_TYPES, block_tq3j, 8, dequantize_tq3j<128>, block_tq3, 8, dequantize_tq3<128>, 128, 128, OP_FLASH_ATTN_EXT_NQPSG, OP_FLASH_ATTN_EXT_NCPSG, true, true>;
template [[host_name("kernel_flash_attn_ext_tq3j_tq2_dk128_dv128")]] kernel flash_attn_ext_t kernel_flash_attn_ext<FA_TYPES, block_tq3j, 8, dequantize_tq3j<128>, block_tq2, 8, dequantize_tq2<128>, 128, 128, OP_FLASH_ATTN_EXT_NQPSG, OP_FLASH_ATTN_EXT_NCPSG, true, true>;
template [[host_name("kernel_flash_attn_ext_tq2j_tq3_dk128_dv128")]] kernel flash_attn_ext_t kernel_flash_attn_ext<FA_TYPES, block_tq2j, 8, dequantize_tq2j<128>, block_tq3, 8, dequantize_tq3<128>, 128, 128, OP_FLASH_ATTN_EXT_NQPSG, OP_FLASH_ATTN_EXT_NCPSG, true, true>;
template [[host_name("kernel_flash_attn_ext_tq2j_tq2_dk128_dv128")]] kernel flash_attn_ext_t kernel_flash_attn_ext<FA_TYPES, block_tq2j, 8, dequantize_tq2j<128>, block_tq2, 8, dequantize_tq2<128>, 128, 128, OP_FLASH_ATTN_EXT_NQPSG, OP_FLASH_ATTN_EXT_NCPSG, true, true>;
// dk256 (per-layer type names — all K/V combos)
template [[host_name("kernel_flash_attn_ext_tq3j_256_tq3_256_dk256_dv256")]] kernel flash_attn_ext_t kernel_flash_attn_ext<FA_TYPES, block_tq3j, 16, dequantize_tq3j<256>, block_tq3, 16, dequantize_tq3<256>, 256, 256, OP_FLASH_ATTN_EXT_NQPSG, OP_FLASH_ATTN_EXT_NCPSG, true, true>;
template [[host_name("kernel_flash_attn_ext_tq3j_256_tq2_256_dk256_dv256")]] kernel flash_attn_ext_t kernel_flash_attn_ext<FA_TYPES, block_tq3j, 16, dequantize_tq3j<256>, block_tq2, 16, dequantize_tq2<256>, 256, 256, OP_FLASH_ATTN_EXT_NQPSG, OP_FLASH_ATTN_EXT_NCPSG, true, true>;
template [[host_name("kernel_flash_attn_ext_tq2j_256_tq3_256_dk256_dv256")]] kernel flash_attn_ext_t kernel_flash_attn_ext<FA_TYPES, block_tq2j, 16, dequantize_tq2j<256>, block_tq3, 16, dequantize_tq3<256>, 256, 256, OP_FLASH_ATTN_EXT_NQPSG, OP_FLASH_ATTN_EXT_NCPSG, true, true>;
template [[host_name("kernel_flash_attn_ext_tq2j_256_tq2_256_dk256_dv256")]] kernel flash_attn_ext_t kernel_flash_attn_ext<FA_TYPES, block_tq2j, 16, dequantize_tq2j<256>, block_tq2, 16, dequantize_tq2<256>, 256, 256, OP_FLASH_ATTN_EXT_NQPSG, OP_FLASH_ATTN_EXT_NCPSG, true, true>;
// dk512 (per-layer type names — all K/V combos)
template [[host_name("kernel_flash_attn_ext_tq3j_512_tq3_512_dk512_dv512")]] kernel flash_attn_ext_t kernel_flash_attn_ext<FA_TYPES, block_tq3j, 32, dequantize_tq3j<512>, block_tq3, 32, dequantize_tq3<512>, 512, 512, OP_FLASH_ATTN_EXT_NQPSG, OP_FLASH_ATTN_EXT_NCPSG, true, true>;
template [[host_name("kernel_flash_attn_ext_tq3j_512_tq2_512_dk512_dv512")]] kernel flash_attn_ext_t kernel_flash_attn_ext<FA_TYPES, block_tq3j, 32, dequantize_tq3j<512>, block_tq2, 32, dequantize_tq2<512>, 512, 512, OP_FLASH_ATTN_EXT_NQPSG, OP_FLASH_ATTN_EXT_NCPSG, true, true>;
...
template [[host_name("kernel_flash_attn_ext_vec_tq3j_tq3_dk128_dv128")]] kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_tq3j, 32, dequantize_tq3j_t4<128>, block_tq3, 32, dequantize_tq3_t4<128>, 128, 128, 1, OP_FLASH_ATTN_EXT_VEC_NQPSG, OP_FLASH_ATTN_EXT_VEC_NCPSG, true>;
template [[host_name("kernel_flash_attn_ext_vec_tq3j_tq2_dk128_dv128")]] kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_tq3j, 32, dequantize_tq3j_t4<128>, block_tq2, 32, dequantize_tq2_t4<128>, 128, 128, 1, OP_FLASH_ATTN_EXT_VEC_NQPSG, OP_FLASH_ATTN_EXT_VEC_NCPSG, true>;
template [[host_name("kernel_flash_attn_ext_vec_tq2j_tq3_dk128_dv128")]] kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_tq2j, 32, dequantize_tq2j_t4<128>, block_tq3, 32, dequantize_tq3_t4<128>, 128, 128, 1, OP_FLASH_ATTN_EXT_VEC_NQPSG, OP_FLASH_ATTN_EXT_VEC_NCPSG, true>;
template [[host_name("kernel_flash_attn_ext_vec_tq2j_tq2_dk128_dv128")]] kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_tq2j, 32, dequantize_tq2j_t4<128>, block_tq2, 32, dequantize_tq2_t4<128>, 128, 128, 1, OP_FLASH_ATTN_EXT_VEC_NQPSG, OP_FLASH_ATTN_EXT_VEC_NCPSG, true>;
template [[host_name("kernel_flash_attn_ext_vec_tq3j_256_tq3_256_dk256_dv256")]] kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_tq3j, 64, dequantize_tq3j_t4<256>, block_tq3, 64, dequantize_tq3_t4<256>, 256, 256, 1, OP_FLASH_ATTN_EXT_VEC_NQPSG, OP_FLASH_ATTN_EXT_VEC_NCPSG, true>;
template [[host_name("kernel_flash_attn_ext_vec_tq3j_256_tq2_256_dk256_dv256")]] kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_tq3j, 64, dequantize_tq3j_t4<256>, block_tq2, 64, dequantize_tq2_t4<256>, 256, 256, 1, OP_FLASH_ATTN_EXT_VEC_NQPSG, OP_FLASH_ATTN_EXT_VEC_NCPSG, true>;
template [[host_name("kernel_flash_attn_ext_vec_tq2j_256_tq3_256_dk256_dv256")]] kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_tq2j, 64, dequantize_tq2j_t4<256>, block_tq3, 64, dequantize_tq3_t4<256>, 256, 256, 1, OP_FLASH_ATTN_EXT_VEC_NQPSG, OP_FLASH_ATTN_EXT_VEC_NCPSG, true>;
template [[host_name("kernel_flash_attn_ext_vec_tq2j_256_tq2_256_dk256_dv256")]] kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_tq2j, 64, dequantize_tq2j_t4<256>, block_tq2, 64, dequantize_tq2_t4<256>, 256, 256, 1, OP_FLASH_ATTN_EXT_VEC_NQPSG, OP_FLASH_ATTN_EXT_VEC_NCPSG, true>;
template [[host_name("kernel_flash_attn_ext_vec_tq3j_512_tq3_512_dk512_dv512")]] kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_tq3j, 128, dequantize_tq3j_t4<512>, block_tq3, 128, dequantize_tq3_t4<512>, 512, 512, 1, OP_FLASH_ATTN_EXT_VEC_NQPSG, OP_FLASH_ATTN_EXT_VEC_NCPSG, true>;
template [[host_name("kernel_flash_attn_ext_vec_tq3j_512_tq2_512_dk512_dv512")]] kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_tq3j, 128, dequantize_tq3j_t4<512>, block_tq2, 128, dequantize_tq2_t4<512>, 512, 512, 1, OP_FLASH_ATTN_EXT_VEC_NQPSG, OP_FLASH_ATTN_EXT_VEC_NCPSG, true>;
template [[host_name("kernel_flash_attn_ext_vec_tq2j_512_tq3_512_dk512_dv512")]] kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_tq2j, 128, dequantize_tq2j_t4<512>, block_tq3, 128, dequantize_tq3_t4<512>, 512, 512, 1, OP_FLASH_ATTN_EXT_VEC_NQPSG, OP_FLASH_ATTN_EXT_VEC_NCPSG, true>;
template [[host_name("kernel_flash_attn_ext_vec_tq2j_512_tq2_512_dk512_dv512")]] kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_tq2j, 128, dequantize_tq2j_t4<512>, block_tq2, 128, dequantize_tq2_t4<512>, 512, 512, 1, OP_FLASH_ATTN_EXT_VEC_NQPSG, OP_FLASH_ATTN_EXT_VEC_NCPSG, true>;

...
// correction is trivial: K_eff[j] = norm*centroid[idx] + (sqrt(pi/2)/d)*rnorm*sign[j]
...
#define NS20 (FC_flash_attn_ext_ns20)
    ...
    for (short jj = 0; jj < NQ; ++jj) {
        ...
        // Inverse FWHT on output: V was stored rotated, so attention output is rotated too.
        if (TQ_FWHT_O) {
            simdgroup_barrier(mem_flags::mem_threadgroup);
...
kernel void kernel_flash_attn_ext(
        ...
        ushort  tiisg[[thread_index_in_simdgroup]],
        ushort  sgitg[[simdgroup_index_in_threadgroup]]) {
    threadgroup uint8_t tq_perm_buf[128];

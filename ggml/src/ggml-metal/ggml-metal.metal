...
// TQK 3.5 dequant for FA template — outputs norm-scaled centroids in rotated space
// Called 8 times per 128-element block (il=0..7, 16 elements each)
// No rotation/QJL — Q is pre-rotated, QJL added via bias
template <typename type4x4>
void dequantize_tqk_35(device const block_tqk_35 * xb, short il, thread type4x4 & reg) {
    float4x4 r;
    if (il < 2) {
        // hi subset: 3-bit, 8 centroids (d=32), elements il*16..(il+1)*16-1
        float norm = (float)xb->norm_hi;
        for (int i = 0; i < 16; i++) {
            int j = il * 16 + i;
            int idx = tq_up3(xb->qs_hi, j);
            r[i/4][i%4] = norm * tq_c8_d32[idx];
        }
    } else {
        // lo subset: 2-bit, 4 centroids (d=96), elements (il-2)*16..(il-2+1)*16-1
        float norm = (float)xb->norm_lo;
        for (int i = 0; i < 16; i++) {
            int j = (il - 2) * 16 + i;
            int idx = (xb->qs_lo[j / 4] >> ((j % 4) * 2)) & 0x3;
            r[i/4][i%4] = norm * tq_c4_d96[idx];
        }
    }
    reg = (type4x4) r;
}
...
void dequantize_tqk_35_t4(device const block_tqk_35 * xb, short il, thread type4 & reg) {
    // il=0..31 (4 elements each, 128 total)
    float4 r;
    int base = il * 4;
    if (base < 32) {
        float norm = (float)xb->norm_hi;
        ...
        for (int i = 0; i < 4; i++) {
            int j = (base - 32) + i;
            int idx = (xb->qs_lo[j / 4] >> ((j % 4) * 2)) & 0x3;
            r[i] = norm * tq_c4_d96[idx];
        }
    }
    reg = (type4) r;
}

// TQV 3.5 dequant for FA template — outputs norm-scaled centroids in rotated space
// 128 elements per block: hi=4-bit (32 channels, 16 centroids), lo=3-bit (96 channels, 8 centroids)
template <typename type4x4>
void dequantize_tqv_35(device const block_tqv_35 * xb, short il, thread type4x4 & reg) {
    float4x4 r;
    if (il < 2) {
        float norm = (float)xb->norm_hi;
        for (int i = 0; i < 16; i++) {
...
template <typename type4>
void dequantize_tqv_35_t4(device const block_tqv_35 * xb, short il, thread type4 & reg) {
    float4 r;
    int base = il * 4;
    if (base < 32) {
        float norm = (float)xb->norm_hi;
...
// TQK35 K + q4_0 V (Q must be pre-rotated, QJL added via mask bias)
template [[host_name("kernel_flash_attn_ext_tqk35_q4_0_dk128_dv128")]] kernel flash_attn_ext_t kernel_flash_attn_ext<FA_TYPES, block_tqk_35, 8, dequantize_tqk_35, block_q4_0, 2, dequantize_q4_0, 128, 128>;
// TQK35 K + TQV35 V (Q pre-rotated, output needs V unrotation)
template [[host_name("kernel_flash_attn_ext_tqk35_tqv35_dk128_dv128")]] kernel flash_attn_ext_t kernel_flash_attn_ext<FA_TYPES, block_tqk_35, 8, dequantize_tqk_35, block_tqv_35, 8, dequantize_tqv_35, 128, 128>;
...
template [[host_name("kernel_flash_attn_ext_vec_tqk35_q4_0_dk128_dv128")]]   kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_tqk_35, 32, dequantize_tqk_35_t4, block_q4_0,   8, dequantize_q4_0_t4,  128, 128, 1>;
template [[host_name("kernel_flash_attn_ext_vec_tqk35_tqv35_dk128_dv128")]]  kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_tqk_35, 32, dequantize_tqk_35_t4, block_tqv_35, 32, dequantize_tqv_35_t4, 128, 128, 1>;
...
// ---------------------------------------------------------------------------
// TurboQuant V cache: get_rows (dequantize) and set_rows (quantize)
// ---------------------------------------------------------------------------

// TQV get_rows — dequantize TQV blocks to float with 128x128 inverse rotation
// Each threadgroup processes one 128-element TQV block.
// Thread layout: 128 threads (4 SIMD groups of 32).
template<typename block_tqv, int BITS_HI, int BITS_LO, int NC_HI, int NC_LO>
kernel void kernel_get_rows_tqv(
        constant ggml_metal_kargs_get_rows & args,
        device const void  * src0      [[buffer(1)]],
        device const void  * src1      [[buffer(2)]],
        device       void  * dst       [[buffer(3)]],
        device const float * rot_inv   [[buffer(4)]],
        uint3               tgpig[[threadgroup_position_in_grid]],
        ushort              tiitg[[thread_index_in_threadgroup]],
        ushort              sgitg[[simdgroup_index_in_threadgroup]]) {

    // Grid: (n_blocks_per_row * ne10, ne11, ne12)
    // Each threadgroup = one 128-element block
    const int32_t nblk = args.ne00 / 128; // blocks per row
    const int32_t iblk = tgpig.x % nblk;
    const int32_t i10  = tgpig.x / nblk;
    const int32_t i11  = tgpig.y;
    const int32_t i12  = tgpig.z;

    // Get row index from src1
    const int32_t r = ((const device int32_t *) ((const device char *) src1 + i12*args.nb12 + i11*args.nb11 + i10*args.nb10))[0];
    ...
    // Source TQV block
    device const block_tqv * blk = (device const block_tqv *) ((const device char *) src0 + i03*args.nb03 + i02*args.nb02 + r*args.nb01) + iblk;

    // Destination float pointer (128 elements per block)
    device float * out = (device float *) ((device char *) dst + i12*args.nb3 + i11*args.nb2 + i10*args.nb1) + iblk * 128;

    const int tid = tiitg; // 0..127

    // Step 1: read norm
    const float norm = half(blk->norm_hi);

    // Step 2: build rotated-space vector from centroid indices
    // Shared memory for the 128-dim rotated vector
    threadgroup float shared_rot[128];

    // Select centroid tables based on template params
    constant float * c_hi = (NC_HI == 16) ? tq_c16 : (NC_HI == 8) ? tq_c8 : tq_c4;
    constant float * c_lo = (NC_LO ==  8) ? tq_c8  : (NC_LO == 4) ? tq_c4 : tq_c2;

    if (tid < 32) {
        // Decode hi centroid for channel tid
        int idx;
        if (BITS_HI == 4) {
            idx = tq_up4(blk->qs_hi, tid);
        } else { // BITS_HI == 3
            idx = tq_up3(blk->qs_hi, tid);
        }
        shared_rot[tid] = c_hi[idx];
    } else {
        // Decode lo centroid for channel (tid - 32) at offset 32
        int j = tid - 32; // 0..95
        int idx;
        if (BITS_LO == 3) {
            idx = tq_up3(blk->qs_lo, j);
        } else { // BITS_LO == 2
            idx = (blk->qs_lo[j / 4] >> ((j % 4) * 2)) & 0x3;
        }
        shared_rot[tid] = c_lo[idx];
    }

    threadgroup_barrier(mem_flags::mem_threadgroup);

    // Step 3: matrix multiply — out[tid] = sum(rot_inv[tid*128 + j] * shared_rot[j])
    float sum = 0.0f;
    device const float * row = rot_inv + tid * 128;
    for (int j = 0; j < 128; j++) {
        sum += row[j] * shared_rot[j];
    }

    // Step 4: scale by norm and write output
    out[tid] = norm * sum;
}

// TQV set_rows — quantize float to TQV blocks with 128x128 forward rotation
template<typename block_tqv, int BITS_HI, int BITS_LO, int NC_HI, int NC_LO>
kernel void kernel_set_rows_tqv(
        constant ggml_metal_kargs_set_rows & args,
        device const void  * src0      [[buffer(1)]],
        device const void  * src1      [[buffer(2)]],
        device       void  * dst       [[buffer(3)]],
        device const float * rot_fwd   [[buffer(4)]],
        uint3               tgpig[[threadgroup_position_in_grid]],
        ushort              tiitg[[thread_index_in_threadgroup]]) {
    ...
    // Block within the row
    const int32_t nblk = args.nk0; // nk0 = ne0 / block_size = ne0 / 128
    const int32_t iblk = tgpig.x % nblk;
    const int32_t i01  = tgpig.x / nblk;
    if (i01 >= args.ne01) {
        return;
    }

    const int32_t i10 = i01;
    const int32_t i1 = ((const device int32_t *) ((const device char *) src1 + i10*args.nb10 + i11*args.nb11 + i12*args.nb12))[0];

    const device float * src_row = (const device float *) ((const device char *) src0 + i01*args.nb01 + i02*args.nb02 + i03*args.nb03) + iblk * 128;
    device block_tqv * blk = (device block_tqv *) ((device char *) dst + i1*args.nb1 + i02*args.nb2 + i03*args.nb3) + iblk;

    const int tid = tiitg; // 0..127

    // Step 1: load input and compute L2 norm via SIMD reduction
    ...
    // Reduce across 4 SIMD groups using shared memory
    threadgroup float partial_sums[4];
    ushort sgitg = tid / 32;
    if (tid % 32 == 0) {
        partial_sums[sgitg] = sq;
    }
    threadgroup_barrier(mem_flags::mem_threadgroup);

    float norm = 0.0f;
    if (tid < 4) {
        norm = partial_sums[tid];
    }
    norm = simd_sum(norm); // now all threads in first SIMD group have the total
    // Broadcast to all threads
    threadgroup float shared_norm[1];
    if (tid == 0) {
        shared_norm[0] = sqrt(norm);
    }
    threadgroup_barrier(mem_flags::mem_threadgroup);
    norm = shared_norm[0];
    ...
    float xhat = shared_input[tid] * inv_norm;

    // Step 3: rotate — rot[tid] = sum(rot_fwd[tid*128+j] * shared_input[j] * inv_norm)
    threadgroup_barrier(mem_flags::mem_threadgroup);
    shared_input[tid] = xhat; // reuse shared memory for normalized input
    threadgroup_barrier(mem_flags::mem_threadgroup);

    float rot_val = 0.0f;
    device const float * row = rot_fwd + tid * 128;
    for (int j = 0; j < 128; j++) {
        rot_val += row[j] * shared_input[j];
    }
    shared_rot[tid] = rot_val;
    threadgroup_barrier(mem_flags::mem_threadgroup);

    // Step 4: quantize — find nearest centroid and pack bits
    // Only thread 0 does the packing to avoid byte-level races
    if (tid == 0) {
        // Store norm
        blk->norm_hi = half(norm);
        blk->norm_lo = half(0.0f);

        constant float * c_hi = (NC_HI == 16) ? tq_c16 : (NC_HI == 8) ? tq_c8 : tq_c4;
        constant float * c_lo = (NC_LO ==  8) ? tq_c8  : (NC_LO == 4) ? tq_c4 : tq_c2;

        // Zero out quant storage
        for (int i = 0; i < (int)sizeof(blk->qs_hi); i++) blk->qs_hi[i] = 0;
        for (int i = 0; i < (int)sizeof(blk->qs_lo); i++) blk->qs_lo[i] = 0;

        // Quantize hi (first 32 rotated channels)
        for (int j = 0; j < 32; j++) {
            int idx = tq_nearest(shared_rot[j], c_hi, NC_HI);
            if (BITS_HI == 4) {
                tq_pk4(blk->qs_hi, j, idx);
            } else { // BITS_HI == 3
                tq_pk3(blk->qs_hi, j, idx);
            }
        }

        // Quantize lo (last 96 rotated channels)
        for (int j = 0; j < 96; j++) {
            int idx = tq_nearest(shared_rot[32 + j], c_lo, NC_LO);
            if (BITS_LO == 3) {
                tq_pk3(blk->qs_lo, j, idx);
            } else { // BITS_LO == 2
                blk->qs_lo[j / 4] |= (uint8_t)(idx << ((j % 4) * 2));
            }
        }
    }
}

// Template instantiations for TQV get_rows
// TQV 2.5: hi=3-bit (8 centroids), lo=2-bit (4 centroids)
template [[host_name("kernel_get_rows_tqv25")]]
kernel void kernel_get_rows_tqv<block_tqv_25, 3, 2, 8, 4>(
    constant ggml_metal_kargs_get_rows &, device const void *, device const void *,
    device void *, device const float *, uint3, ushort, ushort);

// TQV 3.5: hi=4-bit (16 centroids), lo=3-bit (8 centroids)
template [[host_name("kernel_get_rows_tqv35")]]
kernel void kernel_get_rows_tqv<block_tqv_35, 4, 3, 16, 8>(
    constant ggml_metal_kargs_get_rows &, device const void *, device const void *,
    device void *, device const float *, uint3, ushort, ushort);

// Template instantiations for TQV set_rows
template [[host_name("kernel_set_rows_tqv25_i32")]]
kernel void kernel_set_rows_tqv<block_tqv_25, 3, 2, 8, 4>(
    constant ggml_metal_kargs_set_rows &, device const void *, device const void *,
    device void *, device const float *, uint3, ushort);

template [[host_name("kernel_set_rows_tqv35_i32")]]
kernel void kernel_set_rows_tqv<block_tqv_35, 4, 3, 16, 8>(
    constant ggml_metal_kargs_set_rows &, device const void *, device const void *,
    device void *, device const float *, uint3, ushort);

// ---------------------------------------------------------------------------
// TurboQuant K cache: get_rows (dequantize) and set_rows (quantize)
// K cache has: dual rotation (32x32 hi + 96x96 lo), QJL correction,
// per-layer-per-head outlier channel maps
// ---------------------------------------------------------------------------

template<typename block_tqk, int BITS_HI, int BITS_LO, int NC_HI, int NC_LO>
kernel void kernel_get_rows_tqk(
        constant ggml_metal_kargs_get_rows_tqk & args,
        device const void  * src0       [[buffer(1)]],
        device const void  * src1       [[buffer(2)]],
        device       void  * dst        [[buffer(3)]],
        device const float * rot_hi_inv [[buffer(4)]],   // 32x32
        device const float * rot_lo_inv [[buffer(5)]],   // 96x96
        device const float * qjl_hi     [[buffer(6)]],   // 32x32 Gaussian
        device const float * qjl_lo     [[buffer(7)]],   // 96x96 Gaussian
        device const int   * channel_map [[buffer(8)]],  // [n_layers * n_kv_heads * 128]
        uint3               tgpig[[threadgroup_position_in_grid]],
        ushort              tiitg[[thread_index_in_threadgroup]]) {

    const int32_t nblk = args.ne00 / 128;
    const int32_t iblk = tgpig.x % nblk;
    const int32_t i10  = tgpig.x / nblk;
    const int32_t i11  = tgpig.y;
    const int32_t i12  = tgpig.z;
    ...
    device const block_tqk * blk = (device const block_tqk *) ((const device char *) src0 + i03*args.nb03 + i02*args.nb02 + r*args.nb01) + iblk;
    device float * out = (device float *) ((device char *) dst + i12*args.nb3 + i11*args.nb2 + i10*args.nb1) + iblk * 128;

    const int tid = tiitg; // 0..127

    // Per-head channel map: outlier[0..31] then regular[32..127]
    const int head = iblk;
    device const int * chmap = channel_map + ((int64_t)args.layer * args.n_kv_heads + head) * 128;

    // Read norms
    const float norm_hi  = half(blk->norm_hi);
    const float norm_lo  = half(blk->norm_lo);
    const float rnorm_hi = half(blk->rnorm_hi);
    const float rnorm_lo = half(blk->rnorm_lo);

    // Centroid tables
    constant float * c_hi = (NC_HI == 8) ? tq_c8_d32 : tq_c4_d32;
    constant float * c_lo = (NC_LO == 4) ? tq_c4_d96 : tq_c2_d96;

    // Step 1: Decode centroids in rotated space
    threadgroup float shared_hi[32];
    threadgroup float shared_lo[96];
    ...
    if (tid < 32) {
        ...
        if (BITS_HI == 3) {
            ...
            idx = (blk->qs_hi[tid / 4] >> ((tid % 4) * 2)) & 0x3;
        }
        shared_hi[tid] = c_hi[idx];
    }
    if (tid < 96) {
        int idx;
        if (BITS_LO == 2) {
            idx = (blk->qs_lo[tid / 4] >> ((tid % 4) * 2)) & 0x3;
        } else { // BITS_LO == 1
            idx = (blk->qs_lo[tid / 8] >> (tid % 8)) & 1;
        }
        shared_lo[tid] = c_lo[idx];
    }
    threadgroup_barrier(mem_flags::mem_threadgroup);

    // Step 2: Unrotate hi (32x32) and lo (96x96)
    threadgroup float hi_orig[32];
    threadgroup float lo_orig[96];

    if (tid < 32) {
        float sum = 0.0f;
        device const float * row = rot_hi_inv + tid * 32;
        for (int j = 0; j < 32; j++) {
            sum += row[j] * shared_hi[j];
        }
        hi_orig[tid] = sum * norm_hi;
    }
    if (tid < 96) {
        float sum = 0.0f;
        device const float * row = rot_lo_inv + tid * 96;
        for (int j = 0; j < 96; j++) {
            sum += row[j] * shared_lo[j];
        }
        lo_orig[tid] = sum * norm_lo;
    }
    threadgroup_barrier(mem_flags::mem_threadgroup);

    // Step 3: QJL correction
    // CPU qjl_inverse: corr[j] = sum_i(S[i][j] * sign[i]) * scale
    // S is stored row-major: S[i][j] = qjl[i * m + j]
    // So corr[j] = sum_i(qjl[i * m + j] * sign[i]) = column j of S^T * signs
    if (tid < 32 && rnorm_hi != 0.0f) {
        float sum = 0.0f;
        for (int i = 0; i < 32; i++) {
            float sign = ((blk->signs_hi[i / 8] >> (i % 8)) & 1) ? 1.0f : -1.0f;
            sum += qjl_hi[i * 32 + tid] * sign; // column tid of row i = S^T[tid][i]
        }
        hi_orig[tid] += sum * 1.2533141f / 32.0f * rnorm_hi;
    }
    if (tid < 96 && rnorm_lo != 0.0f) {
        float sum = 0.0f;
        for (int i = 0; i < 96; i++) {
            float sign = ((blk->signs_lo[i / 8] >> (i % 8)) & 1) ? 1.0f : -1.0f;
            sum += qjl_lo[i * 96 + tid] * sign; // column tid of row i
        }
        lo_orig[tid] += sum * 1.2533141f / 96.0f * rnorm_lo;
    }
    threadgroup_barrier(mem_flags::mem_threadgroup);

    // Step 4: Merge channels — scatter hi/lo to original positions using channel map
    if (tid < 32) {
        out[chmap[tid]] = hi_orig[tid];
    }
    if (tid < 96) {
        out[chmap[32 + tid]] = lo_orig[tid];
    }
}

template<typename block_tqk, int BITS_HI, int BITS_LO, int NC_HI, int NC_LO>
kernel void kernel_set_rows_tqk(
        constant ggml_metal_kargs_set_rows_tqk & args,
        device const void  * src0       [[buffer(1)]],
        device const void  * src1       [[buffer(2)]],
        device       void  * dst        [[buffer(3)]],
        device const float * rot_hi_fwd [[buffer(4)]],   // 32x32
        device const float * rot_lo_fwd [[buffer(5)]],   // 96x96
        device const float * rot_hi_inv [[buffer(6)]],   // 32x32 (for residual)
        device const float * rot_lo_inv [[buffer(7)]],   // 96x96 (for residual)
        device const float * qjl_hi     [[buffer(8)]],   // 32x32 Gaussian
        device const float * qjl_lo     [[buffer(9)]],   // 96x96 Gaussian
        device const int   * channel_map [[buffer(10)]],
        uint3               tgpig[[threadgroup_position_in_grid]],
        ushort              tiitg[[thread_index_in_threadgroup]]) {
    ...
    const device float * src_row = (const device float *) ((const device char *) src0 + i01*args.nb01 + i02*args.nb02 + i03*args.nb03) + iblk * 128;
    device block_tqk * blk = (device block_tqk *) ((device char *) dst + i1*args.nb1 + i02*args.nb2 + i03*args.nb3) + iblk;

    const int tid = tiitg;

    // Per-head channel map
    const int head = iblk;
    device const int * chmap = channel_map + ((int64_t)args.layer * args.n_kv_heads + head) * 128;

    // All work done by thread 0 for simplicity (correctness first, optimize later)
    if (tid != 0) { return; }
    ...
    for (int i = 0; i < 32; i++) { hi_raw[i] = src_row[chmap[i]]; }
    for (int i = 0; i < 96; i++) { lo_raw[i] = src_row[chmap[32 + i]]; }

    // Step 2: Rotate
    float hi_rot[32], lo_rot[96];
    for (int i = 0; i < 32; i++) {
        float sum = 0.0f;
        for (int j = 0; j < 32; j++) { sum += rot_hi_fwd[i * 32 + j] * hi_raw[j]; }
        hi_rot[i] = sum;
    }
    for (int i = 0; i < 96; i++) {
        float sum = 0.0f;
        for (int j = 0; j < 96; j++) { sum += rot_lo_fwd[i * 96 + j] * lo_raw[j]; }
        lo_rot[i] = sum;
    }
    ...
    for (int i = 0; i < (int)sizeof(blk->qs_hi); i++) { blk->qs_hi[i] = 0; }
    for (int i = 0; i < (int)sizeof(blk->qs_lo); i++) { blk->qs_lo[i] = 0; }
    for (int i = 0; i < (int)sizeof(blk->signs_hi); i++) { blk->signs_hi[i] = 0; }
    for (int i = 0; i < (int)sizeof(blk->signs_lo); i++) { blk->signs_lo[i] = 0; }
    ...
    for (int j = 0; j < 32; j++) {
        ...
        int idx = tq_nearest(xn, c_hi, NC_HI);
        if (BITS_HI == 3) { tq_pk3(blk->qs_hi, j, idx); }
        else { blk->qs_hi[j / 4] |= (uint8_t)(idx << ((j % 4) * 2)); }
        yhi[j] = c_hi[idx];
    }
    for (int j = 0; j < 96; j++) {
        ...
        int idx;
        if (BITS_LO == 2) {
            idx = tq_nearest(xn, c_lo, NC_LO);
            blk->qs_lo[j / 4] |= (uint8_t)(idx << ((j % 4) * 2));
        } else { // BITS_LO == 1
            idx = (xn >= 0.0f) ? 1 : 0;
            blk->qs_lo[j / 8] |= (uint8_t)(idx << (j % 8));
        }
        ylo[j] = c_lo[idx];
    }

    // Step 5: Compute residual in original subset space
    // r = x_raw - norm * Π^T · centroids
    float hi_rec[32], lo_rec[96];
    for (int i = 0; i < 32; i++) {
        float sum = 0.0f;
        for (int j = 0; j < 32; j++) { sum += rot_hi_inv[i * 32 + j] * yhi[j]; }
        hi_rec[i] = sum;
    }
    for (int i = 0; i < 96; i++) {
        float sum = 0.0f;
        for (int j = 0; j < 96; j++) { sum += rot_lo_inv[i * 96 + j] * ylo[j]; }
        lo_rec[i] = sum;
    }

    float r_hi[32], r_lo[96];
    for (int j = 0; j < 32; j++) { r_hi[j] = hi_raw[j] - norm_hi * hi_rec[j]; }
    for (int j = 0; j < 96; j++) { r_lo[j] = lo_raw[j] - norm_lo * lo_rec[j]; }

    // Step 6: QJL forward — sign(S · r) and ||r||
    float rnorm_hi_sq = 0.0f, rnorm_lo_sq = 0.0f;
    for (int j = 0; j < 32; j++) { rnorm_hi_sq += r_hi[j] * r_hi[j]; }
    for (int j = 0; j < 96; j++) { rnorm_lo_sq += r_lo[j] * r_lo[j]; }

    blk->rnorm_hi = half(sqrt(rnorm_hi_sq));
    blk->rnorm_lo = half(sqrt(rnorm_lo_sq));

    for (int i = 0; i < 32; i++) {
        float proj = 0.0f;
        for (int j = 0; j < 32; j++) { proj += qjl_hi[i * 32 + j] * r_hi[j]; }
        if (proj >= 0.0f) { blk->signs_hi[i / 8] |= (uint8_t)(1 << (i % 8)); }
    }
    for (int i = 0; i < 96; i++) {
        float proj = 0.0f;
        for (int j = 0; j < 96; j++) { proj += qjl_lo[i * 96 + j] * r_lo[j]; }
        if (proj >= 0.0f) { blk->signs_lo[i / 8] |= (uint8_t)(1 << (i % 8)); }
    }
}

// TQK template instantiations
// TQK 2.5: hi=2-bit (4 centroids d=32), lo=1-bit (2 centroids d=96)
template [[host_name("kernel_get_rows_tqk25")]]
kernel void kernel_get_rows_tqk<block_tqk_25, 2, 1, 4, 2>(
    constant ggml_metal_kargs_get_rows_tqk &, device const void *, device const void *,
    device void *, device const float *, device const float *, device const float *,
    device const float *, device const int *, uint3, ushort);

// TQK 3.5: hi=3-bit (8 centroids d=32), lo=2-bit (4 centroids d=96)
template [[host_name("kernel_get_rows_tqk35")]]
kernel void kernel_get_rows_tqk<block_tqk_35, 3, 2, 8, 4>(
    constant ggml_metal_kargs_get_rows_tqk &, device const void *, device const void *,
    device void *, device const float *, device const float *, device const float *,
    device const float *, device const int *, uint3, ushort);

template [[host_name("kernel_set_rows_tqk25_i32")]]
kernel void kernel_set_rows_tqk<block_tqk_25, 2, 1, 4, 2>(
    constant ggml_metal_kargs_set_rows_tqk &, device const void *, device const void *,
    device void *, device const float *, device const float *, device const float *,
    device const float *, device const float *, device const float *,
    device const int *, uint3, ushort);

template [[host_name("kernel_set_rows_tqk35_i32")]]
kernel void kernel_set_rows_tqk<block_tqk_35, 3, 2, 8, 4>(
    constant ggml_metal_kargs_set_rows_tqk &, device const void *, device const void *,
    device void *, device const float *, device const float *, device const float *,
    device const float *, device const float *, device const float *,
    device const int *, uint3, ushort);
...
// Pre-rotate Q for TQK attention: split by channel map, apply rot_hi/rot_lo
// Q layout: [head_dim=128, n_queries, n_heads_q]
// Output: rotated Q in-place (same buffer), reordered as [hi_32 | lo_96]
kernel void kernel_tq_rotate_q(
        device       float * Q           [[buffer(0)]],
        device const float * rot_hi_fwd  [[buffer(1)]],
        device const float * rot_lo_fwd  [[buffer(2)]],
        device const int   * channel_map [[buffer(3)]],
        constant int32_t & n_queries     [[buffer(4)]],
        constant int32_t & n_heads_q     [[buffer(5)]],
        constant int32_t & n_heads_kv    [[buffer(6)]],
        constant int32_t & layer         [[buffer(7)]],
        constant uint64_t & nb01         [[buffer(8)]],
        constant uint64_t & nb02         [[buffer(9)]],
        constant int32_t & ne02          [[buffer(10)]],
        constant int32_t & ne_12_2       [[buffer(11)]],
        uint3  tgpig [[threadgroup_position_in_grid]],
        ushort tiitg [[thread_index_in_threadgroup]]) {

    const int iq1 = tgpig.x; // query index
    const int iq2 = tgpig.y; // Q head index
    const int tid = tiitg;    // 0..127

    if (iq1 >= n_queries) return;

    const int ikv2 = iq2 / (ne02 / ne_12_2);
    device const int * chmap = channel_map + ((int64_t)layer * n_heads_kv + ikv2) * 128;

    device float * q_ptr = (device float *)((device char *)Q + iq1*nb01 + iq2*nb02);

    // Load original Q to shared memory
    threadgroup float sq[128];
    sq[tid] = q_ptr[tid];
    threadgroup_barrier(mem_flags::mem_threadgroup);

    // Compute rotated element
    float rotated;
    if (tid < 32) {
        float sum = 0.0f;
        for (int j = 0; j < 32; j++) {
            sum += rot_hi_fwd[tid * 32 + j] * sq[chmap[j]];
        }
        rotated = sum;
    } else {
        int lo_tid = tid - 32;
        float sum = 0.0f;
        for (int j = 0; j < 96; j++) {
            sum += rot_lo_fwd[lo_tid * 96 + j] * sq[chmap[32 + j]];
        }
        rotated = sum;
    }
    threadgroup_barrier(mem_flags::mem_threadgroup);
...
// Precompute QJL bias for each KV token — outputs half values to add to FA mask
// One threadgroup per query × head, 128 threads cooperate on QJL dot products
kernel void kernel_tq_qjl_bias(
        device const float * Q           [[buffer(0)]],   // original (unrotated) Q
        device const char  * K           [[buffer(1)]],   // TQK blocks
        device       half  * bias_out    [[buffer(2)]],   // [n_kv, n_queries] output
        device const float * qjl_hi      [[buffer(3)]],
        device const float * qjl_lo      [[buffer(4)]],
        device const int   * channel_map [[buffer(5)]],
        constant int32_t & ne01          [[buffer(6)]],   // n_queries
        constant int32_t & ne11          [[buffer(7)]],   // n_kv
        constant int32_t & ne02          [[buffer(8)]],   // n_heads_q
        constant int32_t & ne_12_2       [[buffer(9)]],
        constant uint64_t & nb01         [[buffer(10)]],
        constant uint64_t & nb02         [[buffer(11)]],
        constant uint64_t & nb11         [[buffer(12)]],
        constant uint64_t & nb12         [[buffer(13)]],
        constant int32_t & n_heads_kv    [[buffer(14)]],
        constant int32_t & layer         [[buffer(15)]],
        constant float & scale           [[buffer(16)]],
        uint3  tgpig [[threadgroup_position_in_grid]],
        ushort tiitg [[thread_index_in_threadgroup]],
        ushort sgitg [[simdgroup_index_in_threadgroup]]) {

    const int iq1 = tgpig.x;
    ...
    // Load Q and precompute Sq = S @ q_raw (per subset)
    device const float * q_ptr = (device const float *)((device const char *)Q + iq1*nb01 + iq2*nb02);

    threadgroup float sq_raw[128];
    sq_raw[tid] = q_ptr[tid];
    threadgroup_barrier(mem_flags::mem_threadgroup);

    // Each thread computes one element of Sq
    float my_sq;
    if (tid < 32) {
        float sum = 0.0f;
        for (int j = 0; j < 32; j++) sum += qjl_hi[tid * 32 + j] * sq_raw[chmap[j]];
        my_sq = sum;
    } else {
        int lo_tid = tid - 32;
        float sum = 0.0f;
        for (int j = 0; j < 96; j++) sum += qjl_lo[lo_tid * 96 + j] * sq_raw[chmap[32 + j]];
        my_sq = sum;
    }

    // For each KV token, compute QJL bias
    threadgroup float sg_partials[4];
    device const char * k_base = K + ikv2*nb12;

    for (int t = 0; t < ne11; t++) {
        device const block_tqk_35 * k_blk = (device const block_tqk_35 *)(k_base + t*nb11);

        float qjl_partial = 0.0f;
        if (tid < 32) {
            float rnorm = (float)k_blk->rnorm_hi;
            if (rnorm != 0.0f) {
                float sign = ((k_blk->signs_hi[lane / 8] >> (lane % 8)) & 1) ? 1.0f : -1.0f;
                qjl_partial = my_sq * sign * rnorm * (1.2533141f / 32.0f);
            }
        } else {
            float rnorm = (float)k_blk->rnorm_lo;
            if (rnorm != 0.0f) {
                int j = tid - 32;
                float sign = ((k_blk->signs_lo[j / 8] >> (j % 8)) & 1) ? 1.0f : -1.0f;
                qjl_partial = my_sq * sign * rnorm * (1.2533141f / 96.0f);
            }
        }

        float qjl_sg = simd_sum(qjl_partial);
        if (lane == 0) sg_partials[sgitg] = qjl_sg;
        threadgroup_barrier(mem_flags::mem_threadgroup);

        if (tid == 0) {
            float bias = (sg_partials[0] + sg_partials[1] + sg_partials[2] + sg_partials[3]) * scale;
            bias_out[iq1 * ne11 + t] = (half)bias;
        }
        threadgroup_barrier(mem_flags::mem_threadgroup);
    }
}
...
// ---------------------------------------------------------------------------
// TurboQuant Flash Attention — optimized rotation-factored kernel
//
// Optimizations over v1:
//   1. SIMD reduction for K dot product (1 multiply/thread + simd_sum, not 128)
//   2. Direct device reads for K/V centroids (no shared memory for centroids)
//   3. Single barrier per token (partial sum broadcast only)
//   4. QJL via SIMD reduction (distributed, not serial)
//
// Thread layout: 128 threads = 4 SIMD groups of 32
//   SIMD 0: elements 0-31  (K hi subset)
//   SIMD 1: elements 32-63 (K lo subset, first 32)
//   SIMD 2: elements 64-95 (K lo subset, middle 32)
//   SIMD 3: elements 96-127 (K lo subset, last 32)
// ---------------------------------------------------------------------------

kernel void kernel_flash_attn_ext_tq(
        constant ggml_metal_kargs_flash_attn_ext_tq & args [[buffer(0)]],
        ...
        device const float * rot_hi_fwd [[buffer(6)]],
        device const float * rot_lo_fwd [[buffer(7)]],
        device const float * rot_v_inv  [[buffer(8)]],
        device const float * qjl_hi     [[buffer(9)]],
        device const float * qjl_lo     [[buffer(10)]],
        device const int   * channel_map [[buffer(11)]],
        uint3               tgpig[[threadgroup_position_in_grid]],
        ushort              tiitg[[thread_index_in_threadgroup]],
        ushort              sgitg[[simdgroup_index_in_threadgroup]]) {
    ...
    device const int * chmap = channel_map + ((int64_t)args.layer * args.n_kv_heads + ikv2) * 128;

    // === PHASE 1: Load and rotate query ===
    ...
    // Rotate query — each thread computes one element of sq_rot
    // Store in register (not shared memory) — only this thread's element
    float my_q_rot;
    if (tid < 32) {
        float sum = 0.0f;
        for (int j = 0; j < 32; j++) {
            sum += rot_hi_fwd[tid * 32 + j] * sq_raw[chmap[j]];
        }
        my_q_rot = sum;
    } else {
        int lo_tid = tid - 32;
        float sum = 0.0f;
        for (int j = 0; j < 96; j++) {
            sum += rot_lo_fwd[lo_tid * 96 + j] * sq_raw[chmap[32 + j]];
        }
        my_q_rot = sum;
    }

    // Precompute per-thread QJL projection element
    // Sq_hi[tid] for tid<32, Sq_lo[tid-32] for tid>=32
    float my_sq;
    if (tid < 32) {
        float sum = 0.0f;
        for (int j = 0; j < 32; j++) {
            sum += qjl_hi[tid * 32 + j] * sq_raw[chmap[j]];
        }
        my_sq = sum;
    } else {
        int lo_tid = tid - 32;
        float sum = 0.0f;
        for (int j = 0; j < 96; j++) {
            sum += qjl_lo[lo_tid * 96 + j] * sq_raw[chmap[32 + j]];
        }
        my_sq = sum;
    }
    // No barrier needed — my_q_rot and my_sq are per-thread registers

    // === PHASE 2: Attention loop (batched — B tokens per barrier) ===
    constexpr int B = 8;
    threadgroup float sg_partials[4 * B]; // 4 SIMD groups × B tokens
    ...
    for (int t0 = 0; t0 < args.ne11; t0 += B) {
        const int batch = min(B, args.ne11 - t0);

        // --- Batch: compute B SIMD-reduced scores (no barrier needed between tokens) ---
        float sg_total[B];
        for (int b = 0; b < batch; b++) {
            device const block_tqk_35 * k_blk = (device const block_tqk_35 *)(k_base + (t0+b)*args.nb11);

            float k_val;
            if (tid < 32) {
                int idx = tq_up3(k_blk->qs_hi, tid);
                k_val = (float)k_blk->norm_hi * tq_c8_d32[idx];
            } else {
                int j = tid - 32;
                int idx = (k_blk->qs_lo[j / 4] >> ((j % 4) * 2)) & 0x3;
                k_val = (float)k_blk->norm_lo * tq_c4_d96[idx];
            }

            float mse_sg = simd_sum(my_q_rot * k_val);
            ...
            sg_total[b] = mse_sg + simd_sum(qjl_partial);
        }

        // --- One barrier for the whole batch ---
        for (int b = 0; b < batch; b++) {
            if (lane == 0) { sg_partials[b * 4 + sgitg] = sg_total[b]; }
        }
        threadgroup_barrier(mem_flags::mem_threadgroup);

        // --- Process B scores: softmax + V accumulation ---
        for (int b = 0; b < batch; b++) {
            float score = (sg_partials[b*4+0] + sg_partials[b*4+1] + sg_partials[b*4+2] + sg_partials[b*4+3]) * args.scale;

            if (args.has_mask) {
                score += (float)((device const half *)(mask + iq1*args.nb31))[t0+b];
            }

            float new_max = max(M, score);
            float old_scale = exp(M - new_max);
            float new_exp = exp(score - new_max);
            S = S * old_scale + new_exp;
            acc *= old_scale;
            M = new_max;
            ...
            float v_val;
            if (args.v_is_q4_0) {
                device const block_q4_0 * v_blk = (device const block_q4_0 *)(v_base + t*args.nb21) + tid/32;
                int elem = tid % 32;
                float d = (float)v_blk->d;
                uint8_t nibble = (elem < 16) ? (v_blk->qs[elem] & 0xF) : (v_blk->qs[elem - 16] >> 4);
                v_val = ((float)nibble - 8.0f) * d;
            } else {
                device const block_tqv_35 * v_blk = (device const block_tqv_35 *)(v_base + t*args.nb21);
                float v_norm = (float)v_blk->norm_hi;
                if (tid < 32) {
                    v_val = v_norm * tq_c16[tq_up4(v_blk->qs_hi, tid)];
                } else {
                    v_val = v_norm * tq_c8[tq_up3(v_blk->qs_lo, tid - 32)];
                }
            }
            acc += new_exp * v_val;
        }

        threadgroup_barrier(mem_flags::mem_threadgroup); // before next batch
    }

    // === PHASE 3: Unrotate V output (only for TQV, skip for q4_0) ===
    if (S > 0.0f) { acc /= S; }
    float out_val;
    if (args.v_is_q4_0) {
        out_val = acc;
    } else {
        threadgroup float acc_rot[128];
        acc_rot[tid] = acc;
        threadgroup_barrier(mem_flags::mem_threadgroup);

        out_val = 0.0f;
        device const float * rot_row = rot_v_inv + tid * 128;
        for (int j = 0; j < 128; j++) {
            out_val += rot_row[j] * acc_rot[j];
        }
    }

    uint64_t dst_offset = ((uint64_t)iq3*args.ne2*args.ne1 + (uint64_t)iq2 + (uint64_t)iq1*args.ne1)*128 + tid;
    dst[dst_offset] = out_val;
}
